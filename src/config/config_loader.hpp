...
struct Config {
    ...
    struct GatewayConfig {
        int http_port = 8080;
        int ws_port = 8092;  // WebSocket 端口，默认 8092 避免与 ffmpeg 或其他服务冲突
        std::string log_level = "info";
        std::string log_file = "logs/gateway.log";
        int total_bandwidth_mbps = 0;  // 总带宽（Mbps），0表示未配置，将使用系统检测或默认值
    ...
    struct GB28181Config {
        ...
        int stream_start_timeout_seconds = 60;           // 流启动超时时间（秒），从发送INVITE到收到200 OK的超时
    } gb28181;
    ...
    struct QUICConfig {
        ...
        // FEC 配置
        struct FECConfig {
            std::string algorithm = "smpte_2022_1";  // smpte_2022_1, raptorq, reed_solomon
