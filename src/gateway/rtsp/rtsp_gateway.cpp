...
RTSPGateway::RTSPGateway(
  ...
  // zlm_rtsp_url_ 这个变量名可能有点误导，实际上我们构建的是 RTMP URL 在 helper
  // 中。 这里我们初始化一个 RTMP URL base
  ...
  auto stream_info_detector =
      std::make_unique<gateway::utils::StreamInfoDetector>(ffprobe_path_);
...
RTSPGateway::~RTSPGateway() {
  ...
  for (auto &pair : streams_) {
    std::string stream_id =
        GenerateStreamId(pair.second.target_app, pair.second.target_stream);
...
Result<void> RTSPGateway::Start(const std::string &source_url,
  ...
  LOG_INFO("[RTSP Gateway] AddStream Called for: {} -> {}", source_url,
           stream_id);
  ...
  auto result = smart_processor_->ProcessStream(
      source_url, target_app, target_stream, output_protocol, "rtsp",
      "rtsp_gateway",
      // 直接代理回调
      [this, output_protocol](const std::string &app, const std::string &stream,
                              const std::string &url) {
        // RTSP Direct Proxy Logic
        // If output is WebRTC, we force FFmpeg to ensure strict H.264+AAC/Opus
        // transcoding and resolution control. For other outputs (FLV, HLS,
        // RTSP), if SmartStreamProcessor detected compatibility, we use Direct
        // Proxy (AddStreamProxy).
        /*
        if (output_protocol == "webrtc") {
             LOG_INFO("[RTSP Gateway] WebRTC output requested, forcing FFmpeg
        for strict transcoding"); return false;
        }
        */
      ...
      [this](const std::string &app, const std::string &stream,
        ...
        return zlm_client_ ? zlm_client_->GetStreamInfo(
                                 app, stream, schema.empty() ? "rtsp" : schema)
                           : streaming::StreamInfo();
      },
      // 启动 FFmpeg 回调
      [this, &info, stream_id, output_protocol](
        ...
        std::string log_file = "/tmp/ffmpeg_rtsp_" + info.target_app + "_" +
  ...
  if (result.success) {
    ...
    // Note: result struct definition needs checking, assuming it has members
    // like success Based on RTMP error, it might NOT have 'message'. I will use
    // generic error for now or look for error field.
    return Result<void>::Failure(gateway::InternalServerException(
        "Failed to start RTSP stream: Process failed"));
  }
}
...
Result<void> RTSPGateway::Stop(const std::string &target_app,
  ...
  auto it = streams_.find(stream_id);
  if (it == streams_.end()) {
    LOG_WARN("[RTSP Gateway] 停止流失败: 流不存在 {}/{}", target_app,
             target_stream);
    if (zlm_client_) {
  ...
  if (success) {
    ...
    return Result<void>::Failure(
        gateway::InternalServerException("Failed to stop RTSP stream"));
  }
}
...
bool RTSPGateway::IsRunning(const std::string &target_app,
  ...
  bool is_running = ::utils::StreamStatusChecker::CheckIsRunning(
      info.pid, info.status, zlm_client_, target_app, target_stream, new_status,
      new_pid);
...
GatewayStatus RTSPGateway::GetStatus(const std::string &target_app,
  ...
  if (new_status == GatewayStatus::Error &&
      info.status != GatewayStatus::Error) {
    LOG_WARN(
        "[RTSP Gateway] FFmpeg 进程已退出，流状态设为 Error: {}/{} (PID: {})",
