...
namespace {
static gateway::utils::AutoRegister reg("psia", [](const gateway::utils::GatewayContext& ctx) -> std::shared_ptr<gateway::GatewayBase> {
    if (ctx.config->psia.enabled) {
        return std::make_shared<PSIAGateway>(ctx.config, ctx.zlm_client, ctx.process_manager, ctx.stream_manager);
    }
    return nullptr;
});
}

PSIAGateway::PSIAGateway(std::shared_ptr<config::Config> config,
    ...
    LOG_INFO("PSIA Gateway 初始化完成（支持直接代理和转码）");
}

PSIAGateway::~PSIAGateway() {
    // 停止所有流
...
Result<void> PSIAGateway::Start(const std::string& source_url,
                       const std::string& target_app,
                       const std::string& target_stream,
                       const std::string& /* output_protocol */) {
    // source_url 格式: psia://device_id[/channel_id]
    // 例如: psia://device_123 或 psia://device_123/1
    
    if (!::utils::url_parser::ValidateURL(source_url, "psia://")) {
        return Result<void>::Failure(gateway::InvalidParameterException("Invalid source URL format: " + source_url));
    }
    
    auto [device_id, channel_id] = ::utils::url_parser::ParseDeviceURL(source_url, "psia://");
    
    return StartDeviceStream(device_id, channel_id, target_app, target_stream);
}

Result<void> PSIAGateway::Stop(const std::string& target_app,
                       const std::string& target_stream) {
    return StopDeviceStream(target_app, target_stream);
}

bool PSIAGateway::IsRunning(const std::string& target_app,
                            const std::string& target_stream) {
...
GatewayStatus PSIAGateway::GetStatus(const std::string& target_app,
                                     const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
    std::lock_guard<std::mutex> lock(streams_mutex_);
    auto it = streams_.find(stream_id);
...
std::vector<PSIADevice> PSIAGateway::DiscoverDevices(const std::string& ip_range,
                                                     int port,
                                                     int timeout_seconds) {
    LOG_INFO("开始发现 PSIA 设备，超时时间: {} 秒", timeout_seconds);
    ...
    LOG_INFO("发现 {} 个 PSIA 设备，已保存到设备列表", devices.size());
    return devices;
}

std::vector<PSIADevice> PSIAGateway::ListDevices() const {
    std::lock_guard<std::mutex> lock(devices_mutex_);
    
    std::vector<PSIADevice> result;
...
PSIADevice PSIAGateway::GetDevice(const std::string& device_id) const {
    std::lock_guard<std::mutex> lock(devices_mutex_);
    
    auto it = devices_.find(device_id);
    if (it == devices_.end()) {
        return PSIADevice();  // 返回空对象
    }
    
    return it->second;
}

std::string PSIAGateway::AddDevice(const PSIADevice& device) {
    std::lock_guard<std::mutex> lock(devices_mutex_);
...
bool PSIAGateway::RemoveDevice(const std::string& device_id) {
    std::lock_guard<std::mutex> lock(devices_mutex_);
...
bool PSIAGateway::UpdateDeviceCredentials(const std::string& device_id,
...
std::vector<std::string> PSIAGateway::GetDeviceRTSPURLs(const std::string& device_id,
                                                         const std::string& channel_id) {
    PSIADevice device = GetDevice(device_id);
    if (device.id.empty()) {
        LOG_ERROR("设备不存在: {}", device_id);
        return {};
    }
...
Result<void> PSIAGateway::StartDeviceStream(const std::string& device_id,
                                    const std::string& channel_id,
                                    const std::string& target_app,
                                    const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
    ...
    auto result = smart_processor_->ProcessStream(
        rtsp_url, target_app, target_stream, "", "rtsp", "psia_gateway",
        // 直接代理回调
        ...
        [this, &info, stream_id](const gateway::utils::StreamInfoResult& stream_info_result) {
            ...
            int recommended_bitrate = bitrate_helper_->GetRecommendedBitrate(
                info.target_app, info.target_stream, "psia", "",
            ...
            LOG_DEBUG("[PSIA Gateway] FFmpeg 命令: {}", command);
            
            int pid = 0;
            GatewayStatus status = GatewayStatus::Stopped;
            
            std::string log_file = "/tmp/ffmpeg_psia_" + info.target_app + "_" + info.target_stream + ".log";
    ...
    if (result.success) {
        ...
        LOG_INFO("PSIA Gateway 启动成功: 设备 {} -> {}/{}", device_id, target_app, target_stream);
        return Result<void>::Success();
    } else {
...
Result<void> PSIAGateway::StopDeviceStream(const std::string& target_app,
                                   const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
...
std::string PSIAGateway::BuildFFmpegCommand(const StreamInfo& info, 
    ...
    if (video_only_transcode) {
        ...
        LOG_INFO("[PSIAGateway] Video({}) is compatible, transcoding audio only, using -c:v copy", 
    ...
    if (stream_info_result.audio_codec == "aac") {
        ...
        LOG_INFO("[PSIAGateway] 源流音频是 AAC，使用 -c:a copy 保持质量");
    } else {
        oss << " -c:a aac -b:a 128k -ar 44100 -ac 2";  // 只有不是 AAC 时才转码
        LOG_INFO("[PSIAGateway] 源流音频是 {}，转码为 AAC", stream_info_result.audio_codec.empty() ? "(unknown)" : stream_info_result.audio_codec);
    }
    
    // 输出参数 - 统一使用 RTMP/FLV 推流格式
    oss << " -f flv"                    // 输出格式为 FLV (推送到 ZLM)
        << " \"" << ::utils::zlm_url_builder::BuildRTMPUrlWithSecret(
            config_, info.target_app, info.target_stream, "PSIA Gateway") << "\"";
    
    return oss.str();
}


std::vector<PSIADevice> PSIAGateway::DiscoverDevicesHTTP(const std::string& ip_range,
                                                         int port,
                                                         int /* timeout_seconds */) {
    std::vector<PSIADevice> devices;
    ...
    for (const auto& ip : ip_list) {
        PSIADevice device = GetDeviceInfo("http://" + ip + ":" + std::to_string(port), "", "");
        if (!device.id.empty()) {
...
std::string PSIAGateway::SendHTTPRequest(const std::string& url,
    ...
    try {
        ...
        if (res != CURLE_OK) {
            LOG_ERROR("PSIA 请求失败: {} (CURL error: {})", url, curl_easy_strerror(res));
            return "";
        }
        
        if (response_code != 200) {
            LOG_ERROR("PSIA 请求返回错误状态码: {} (HTTP {})", url, response_code);
            return "";
        }
        
        LOG_DEBUG("PSIA 请求成功: {} (HTTP {}), 响应长度: {} 字节", url, response_code, response_data.length());
        return response_data;
    } catch (const std::exception& e) {
        LOG_ERROR("PSIA 请求异常: {} (异常信息: {})", url, e.what());
        curl_slist_free_all(headers);
        curl_easy_cleanup(curl);
        return "";
    } catch (...) {
        LOG_ERROR("PSIA 请求未知异常: {}", url);
        curl_slist_free_all(headers);
        curl_easy_cleanup(curl);
        return "";
    }
...
PSIADevice PSIAGateway::GetDeviceInfo(const std::string& base_url,
                                      const std::string& username,
                                      const std::string& password) {
    // PSIA 设备信息 API: /PSIA/System/deviceInfo
    std::string url = base_url + "/PSIA/System/deviceInfo";
    ...
    // 示例响应格式：
    // <deviceInfo>
    //   <deviceName>PSIA Camera</deviceName>
    //   <model>IPC-1234</model>
    ...
    std::regex manufacturer_regex(R"(<manufacturer[^>]*>([^<]+)</manufacturer>)");
    
    std::smatch match;
    if (std::regex_search(response, match, name_regex)) {
        device.manufacturer = match[1].str();
    }
    if (std::regex_search(response, match, manufacturer_regex)) {
        device.manufacturer = match[1].str();
    }
...
std::vector<std::string> PSIAGateway::GetDeviceChannels(const std::string& base_url,
                                                        const std::string& username,
                                                        const std::string& password) {
    // PSIA 通道列表 API: /PSIA/Streaming/channels
    std::string url = base_url + "/PSIA/Streaming/channels";
    std::string response = SendHTTPRequest(url, "GET", "", username, password);
...
std::string PSIAGateway::GetStreamUri(const std::string& base_url,
                                      const std::string& channel_id,
                                      const std::string& username,
                                      const std::string& password) {
    // 优先尝试从 API 获取 RTSP URL（如果设备支持）
    std::string url = base_url + "/PSIA/Streaming/channels/" + channel_id + "/url";
    std::string response = SendHTTPRequest(url, "GET", "", username, password);
    
    if (!response.empty()) {
        // 解析 XML 响应，提取 RTSP URL
        // 示例: <url>rtsp://192.168.1.100:554/stream1</url>
    ...
    // PSIA 设备 RTSP 流地址格式: rtsp://ip:554/stream{channel_id}
    ...
    // 检测是否是模拟器（127.0.0.1:8084），如果是则使用 8554 端口的模拟流
    if (ip == "127.0.0.1" && base_url.find(":8084") != std::string::npos) {
    ...
    // 真实设备：使用标准 PSIA RTSP URL 格式
    std::string rtsp_url = "rtsp://" + ip + ":554/stream" + channel_id;
    if (!username.empty()) {
        rtsp_url = "rtsp://" + username + ":" + password + "@" + ip + ":554/stream" + channel_id;
    }
    
    return rtsp_url;
}

std::string PSIAGateway::GenerateDeviceID(const std::string& base_url) const {
    std::hash<std::string> hasher;
    size_t hash = hasher(base_url);
    std::ostringstream oss;
    oss << "psia_device_" << std::hex << hash;
    return oss.str();
}

std::vector<std::string> PSIAGateway::ParseIPRange(const std::string& ip_range) const {
