#ifndef GATEWAY_PSIA_PSIA_GATEWAY_HPP
#define GATEWAY_PSIA_PSIA_GATEWAY_HPP
 ...
 *
 * 功能：
 * 1. 发现 PSIA 设备（通过 HTTP API 扫描或手动添加）
 * 2. 调用 PSIA API 获取设备的 RTSP 流地址
 * 3. 管理设备的流（直接推流到 ZLMediaKit，因为 RTSP 是原生协议）
 */
class PSIAGateway : public GatewayBase {
public:
    /**
     * @brief 构造函数
     * @param config 配置对象
     ...
     */
    PSIAGateway(std::shared_ptr<config::Config> config,
    ...
    std::string GetProtocol() const override { return "psia"; }
     ...
     * @return 发现的设备列表
     */
    std::vector<PSIADevice> DiscoverDevices(const std::string& ip_range = "", int port = 80, int timeout_seconds = 5);

    /**
     * @brief 获取所有设备列表
     * @return 设备列表
     */
    std::vector<PSIADevice> ListDevices() const;
     ...
     */
    PSIADevice GetDevice(const std::string& device_id) const;
     ...
     * @return 设备 ID
     */
    std::string AddDevice(const PSIADevice& device);
...
private:
    ...
    std::map<std::string, PSIADevice> devices_; // key: device_id
     ...
     * @return 发现的设备列表
     */
    std::vector<PSIADevice> DiscoverDevicesHTTP(const std::string& ip_range,
                                                int port,
                                                int timeout_seconds);
    ...
    /**
     * @brief 获取设备信息 (PSIA /PSIA/System/deviceInfo)
     * @param base_url 设备基础 URL
     * @param username 用户名
     * @param password 密码
     * @return 设备信息
     */
    PSIADevice GetDeviceInfo(const std::string& base_url,
                             const std::string& username,
                             const std::string& password);

    /**
     * @brief 获取设备通道列表 (PSIA /PSIA/Streaming/channels)
     * @param base_url 设备基础 URL
     * @param username 用户名
     * @param password 密码
     * @return 通道 ID 列表
...
#endif // GATEWAY_PSIA_PSIA_GATEWAY_HPP
