 ...
 * @brief SIP 消息类型
 */
enum class SipMethod { REGISTER, INVITE, BYE, MESSAGE, ACK, CANCEL, UNKNOWN };
...
struct SipMessage {
   ...
   * @brief 检查是否是响应消息
   */
  bool IsResponse() const { return status_code > 0; }
};
...
public:
   ...
   */
  static std::string
  BuildResponse(const SipMessage &request, int status_code,
   ...
   */
  static std::string
  BuildRequest(SipMethod method, const std::string &uri,
   ...
   */
  static bool ExtractContactInfo(const std::string &contact_header,
                                 std::string &ip, int &port);

  /**
   * @brief 解析Via头部，提取IP和端口
   */
  static bool ExtractViaInfo(const std::string &via_header, std::string &ip,
                             int &port);
   ...
   */
  static bool ParseAuthorizationHeader(const std::string &auth_header,
                                       std::string &username,
   ...
   */
  static bool VerifyDigestAuth(const std::string &username,
...
private:
   ...
   * @brief 解析头部字段
   */
  static void ParseHeaders(const std::vector<std::string> &lines,
                           SipMessage &msg);
