    ...
    reg("gb28181",
        [](const gateway::utils::GatewayContext &ctx)
          ...
          if (ctx.config->gb28181.enabled) {
            return std::make_shared<GB28181Gateway>(ctx.config, ctx.zlm_client,
                                                    ctx.stream_manager);
          }
          return nullptr;
        });
...
GB28181Gateway::GB28181Gateway(
    ...
    : config_(config), zlm_client_(zlm_client), stream_manager_(stream_manager),
      heartbeat_check_running_(false), stream_status_check_running_(false) {
  ...
  if (config_) {
    ...
    stream_start_timeout_seconds_ =
        config_->gb28181.stream_start_timeout_seconds;
  } else {
    // 使用默认值
  ...
  sip_server_ =
      std::make_unique<SipServer>(local_ip, local_port, server_id_, domain_);

  // 设置SIP服务器回调
  sip_server_->SetDeviceRegisterCallback(
      [this](const std::string &device_id, const std::string &ip, int port,
             int expires) { OnDeviceRegister(device_id, ip, port, expires); });

  sip_server_->SetDeviceHeartbeatCallback(
      [this](const std::string &device_id) { OnDeviceHeartbeat(device_id); });

  // 设置设备认证回调
  sip_server_->SetDeviceAuthCallback([this](const std::string &device_id,
                                            std::string &username,
                                            std::string &password) -> bool {
    GB28181Device device = GetDevice(device_id);
    if (!device.id.empty() && !device.username.empty() &&
        !device.password.empty()) {
  ...
  sip_server_->SetResponseCallback(
      [this](const std::string &call_id, int status_code,
             const std::string &reason_phrase, int peer_port) {
...
GB28181Gateway::~GB28181Gateway() {
  ...
  for (auto &pair : streams_) {
    ...
    if (!info.call_id.empty()) {
      ...
      if (!device.id.empty()) {
        int device_port =
            info.device_sip_port > 0 ? info.device_sip_port : device.port;
        LOG_DEBUG("GB28181 Gateway: 析构时发送BYE请求，使用端口: {} "
                  "(StreamInfo.device_sip_port={}, device.port={})",
...
bool GB28181Gateway::StartSipServer() {
  ...
  heartbeat_check_thread_ =
      std::thread(&GB28181Gateway::CheckDeviceHeartbeat, this);

  // 启动流状态检查线程
  stream_status_check_running_ = true;
  stream_status_check_thread_ =
      std::thread(&GB28181Gateway::CheckStreamStatus, this);
...
Result<void> GB28181Gateway::Start(const std::string &source_url,
    ...
    if (!StartSipServer()) {
      ...
      if (stream_manager_) {
        ...
        stream_manager_->OnStreamCreateResult(
            target_app, target_stream, false, ErrorCode::ZLM_API_FAILED,
            ErrorMessageMapper::GetMessage(ErrorCode::ZLM_API_FAILED));
      }
      return Result<void>::Failure(
          gateway::InternalServerException("Failed to start SIP server"));
    }
  ...
  if (!::utils::url_parser::ValidateURL(source_url, "gb28181://")) {
    ...
    return Result<void>::Failure(gateway::InvalidParameterException(
        "Invalid source URL format: " + source_url));
  }

  auto [device_id, channel_id] =
      ::utils::url_parser::ParseDeviceURL(source_url, "gb28181://");
  if (device_id.empty() || channel_id.empty()) {
    LOG_ERROR("GB28181 Gateway: 无效的source_url（缺少设备ID或通道ID）: {}",
              source_url);
    if (stream_manager_) {
      using namespace gateway::utils;
    ...
    return Result<void>::Failure(gateway::InvalidParameterException(
        "Invalid source URL (missing device/channel ID): " + source_url));
  }

  LOG_DEBUG("GB28181 Gateway: 准备启动流，设备ID={}, 通道ID={}, target_app={}, "
            "target_stream={}",
            device_id, channel_id, target_app, target_stream);

  return StartDeviceStream(device_id, channel_id, target_app, target_stream,
                           output_protocol, source_url);
}

Result<void> GB28181Gateway::StartDeviceStream(
    const std::string &device_id, const std::string &channel_id,
    const std::string &target_app, const std::string &target_stream,
    const std::string &output_protocol, const std::string &source_url) {
  LOG_DEBUG("GB28181 Gateway: StartDeviceStream 开始，设备ID={}, 通道ID={}, "
  ...
  if (device.id.empty()) {
    ...
    LOG_ERROR("GB28181 Gateway: 设备不存在: {} (当前已注册设备数量: {})",
              device_id, device_count);
    if (stream_manager_) {
      using namespace gateway::utils;
      stream_manager_->OnStreamCreateResult(
          target_app, target_stream, false, ErrorCode::DEVICE_NOT_FOUND,
          ErrorMessageMapper::GetMessage(ErrorCode::DEVICE_NOT_FOUND));
    }
    return Result<void>::Failure(
        gateway::DeviceNotFoundException("Device not found: " + device_id));
  }
  ...
  if (!device.online) {
    LOG_WARN("GB28181 Gateway: 设备离线，但允许尝试启动流（用于测试）: {}",
             device_id);
      ...
      [this](StreamInfo &info) {
        ...
        if (!info.rtp_stream_id.empty() && zlm_client_) {
          try {
            ...
            LOG_WARN("GB28181 Gateway: 关闭旧RTP服务器失败: {} - {}",
                     info.rtp_stream_id, e.what());
          }
        }
  ...
  try {
    ...
    for (const auto &server : existing_servers) {
      // 检查完全匹配或app名称匹配
      if (server.stream_id == rtp_stream_id || server.stream_id == target_app ||
        ...
        LOG_INFO("GB28181 Gateway: 发现可能匹配的RTP服务器: "
                 "server_stream_id={}, our_stream_id={}, port={}",
                 server.stream_id, rtp_stream_id, server.port);
        // 尝试关闭所有匹配的服务器
        zlm_client_->CloseRtpServer(server.stream_id, target_app,
                                    "__defaultVhost__");
      }
    }
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(time::GB28181_INVITE_WAIT_MS));
    ...
    for (const auto &server : servers_after_close) {
      if (server.stream_id == rtp_stream_id || server.stream_id == target_app ||
          rtp_stream_id.find(server.stream_id) != std::string::npos) {
        still_exists = true;
        LOG_WARN("GB28181 Gateway: RTP服务器仍然存在: server_stream_id={}, "
                 "our_stream_id={}",
    ...
    if (!still_exists) {
      ...
      // 如果仍然存在，再等待一段时间
      std::this_thread::sleep_for(
          std::chrono::milliseconds(time::GB28181_BYE_WAIT_MS));
    }
  } catch (const std::exception &e) {
    LOG_WARN(
        "GB28181 Gateway: 检查/关闭RTP服务器时出现异常: {}，将继续尝试创建",
        e.what());
  }
  ...
  if (!CreateRtpServerWithRetry(rtp_stream_id, rtp_info, target_app)) {
    ...
    return Result<void>::Failure(
        gateway::ZLMAPIException("Failed to create RTP server"));
  }
  ...
  if (!SendInvite(device, channel_id, rtp_info, call_id)) {
    ...
    return Result<void>::Failure(
        gateway::SIPException("Failed to send SIP INVITE"));
  }
  ...
  auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now() - info.created_time)
                        .count();
  LOG_INFO("GB28181 Gateway: 流创建完成，等待设备响应: {}/{} (Call-ID: {}, "
           "设备: {}, 通道: {}, RTP端口: {}, 已耗时: {}ms, 超时时间: {}秒)",
           target_app, target_stream, call_id, device_id, channel_id,
           rtp_info.port, elapsed_ms, stream_start_timeout_seconds_);
  ...
  if (stream_manager_) {
    ...
    GatewayBase::RegisterStreamToManager(
        stream_manager_, target_app,
        target_stream, // 使用target_stream注册，但ZLM中的流名称是rtp_stream_id
        "gb28181", output_protocol, source_url, "gb28181_gateway",
...
bool GB28181Gateway::CreateRtpServerWithRetry(
    const std::string &rtp_stream_id, streaming::RtpServerInfo &rtp_info,
    const std::string &target_app) {
  // 从配置中读取重试参数（与其他Gateway保持一致）
  int max_retries = config_ ? config_->gateway.direct_proxy.max_retries : 5;
  int retry_interval_ms =
      config_ ? config_->gateway.direct_proxy.retry_interval_ms : 2000;
  ...
  for (int i = 0; i < max_retries; ++i) {
    ...
    std::string local_ip = config_ ? config_->gb28181.local_ip : "0.0.0.0";
    rtp_info =
        zlm_client_->OpenRtpServer(rtp_stream_id,
                                   0, // 自动分配端口
                                   default_tcp_mode_, default_enable_rtcp_,
    ...
    if (i < max_retries - 1) {
      LOG_WARN("GB28181 Gateway: 创建RTP服务器失败，重试 {}/{}", i + 1,
               max_retries);

      // 在重试前，强制尝试关闭可能存在的服务器（即使ListRtpServer没找到）
      // 因为ZLM可能内部状态不一致，需要多次尝试关闭并等待更长时间
      try {
        LOG_DEBUG("GB28181 Gateway: 重试前强制关闭RTP服务器: {}",
                  rtp_stream_id);
        ...
        for (int close_attempt = 0; close_attempt < 5; ++close_attempt) {
          ...
          std::this_thread::sleep_for(std::chrono::milliseconds(
              time::GB28181_RETRY_INTERVAL_MS)); // 每次等待
          ...
          for (const auto &server : servers) {
            if (server.stream_id == rtp_stream_id) {
              found = true;
              LOG_DEBUG(
                  "GB28181 Gateway: RTP服务器仍存在（尝试 {}/5），继续关闭: {}",
                  close_attempt + 1, rtp_stream_id);
              break;
            }
          }
          if (!found) {
            LOG_DEBUG("GB28181 Gateway: RTP服务器已成功关闭: {}",
                      rtp_stream_id);
            break;
          }
        }
      } catch (const std::exception &e) {
        LOG_DEBUG("GB28181 Gateway: 重试前关闭RTP服务器时出现异常: {}",
                  e.what());
      }

      // 等待更长时间，确保ZLM内部状态完全清理
      std::this_thread::sleep_for(std::chrono::milliseconds(
          retry_interval_ms + time::GB28181_RETRY_INTERVAL_MS));
    }
  }

  // 使用错误推断工具
  std::string error_msg =
      "创建RTP服务器失败（已重试" + std::to_string(max_retries) + "次）";
  using namespace gateway::utils;
  auto [error_code, error_message] = InferErrorFromMessage(error_msg);
...
Result<void> GB28181Gateway::Stop(const std::string &target_app,
  ...
  // 2.
  // target_stream是完整名称：34020000001320000001_34020000001320000001_1765384946019

  // 首先尝试去掉时间戳，获取基础名称
  std::string base_stream = target_stream;
  size_t last_underscore = target_stream.find_last_of('_');
  if (last_underscore != std::string::npos &&
      last_underscore < target_stream.length() - 1) {
    ...
    if (is_timestamp) {
      ...
      LOG_DEBUG("GB28181 Gateway: Stop - 检测到时间戳，基础流名称: {} -> {}",
                target_stream, base_stream);
    }
  }
  ...
  std::vector<std::pair<std::string, StreamInfo>>
      candidate_streams; // <stream_id, StreamInfo>
  ...
  if (actual_stream_id.empty() && !candidate_streams.empty()) {
    LOG_DEBUG("GB28181 Gateway: 直接查找失败，尝试匹配流名称: {}/{} (base: {})",
              target_app, target_stream, base_stream);
    for (const auto &candidate : candidate_streams) {
      ...
      if (stream_info.rtp_stream_id == target_stream) {
        ...
        LOG_DEBUG("GB28181 Gateway: 匹配成功（rtp_stream_id == "
                  "target_stream），找到流: {}",
                  target_stream);
        break;
      }
      ...
      if (!rtp_base.empty()) {
        ...
        if (rtp_last_underscore != std::string::npos) {
          std::string rtp_without_timestamp =
              rtp_base.substr(0, rtp_last_underscore);
          if (rtp_without_timestamp == base_stream ||
              rtp_without_timestamp == target_stream) {
            info = stream_info;
            actual_stream_id = candidate.first;
            LOG_DEBUG(
                "GB28181 Gateway: 匹配成功（去掉时间戳），找到流: {} -> {}",
                target_stream, rtp_base);
            break;
          }
        }
      }
      // 检查target_stream或base_stream是否匹配target_stream
      if (stream_info.target_stream == target_stream ||
          stream_info.target_stream == base_stream) {
        info = stream_info;
        actual_stream_id = candidate.first;
  ...
  if (actual_stream_id.empty()) {
    LOG_DEBUG("GB28181 Gateway: streams_ map中未找到流，尝试从ZLM查询: {}/{} "
              "(base: {})",
              target_app, target_stream, base_stream);
    if (zlm_client_) {
      try {
        // 尝试获取rtp_stream_id（可能从ZLM查询到）
        // 注意：GetRtpStreamId内部会加锁，但不会调用ZLM
        // API（已禁用），所以不会阻塞
        std::string rtp_stream_id = GetRtpStreamId(target_app, base_stream);
        if (!rtp_stream_id.empty()) {
          // 找到了rtp_stream_id，尝试关闭RTP服务器
          LOG_INFO("GB28181 Gateway: 从ZLM查询到流，尝试关闭RTP服务器: {}",
                   rtp_stream_id);
          try {
            zlm_client_->CloseRtpServer(rtp_stream_id);
            LOG_INFO("GB28181 Gateway: 已关闭RTP服务器（流不在Gateway streams_ "
                     "map中）: {}",
                     rtp_stream_id);
          } catch (const std::exception &e) {
            LOG_WARN("GB28181 Gateway: 关闭RTP服务器失败: {} - {}",
                     rtp_stream_id, e.what());
          }
          // 从StreamManager注销流
          if (stream_manager_) {
          ...
          return Result<void>::
              Success(); // 即使streams_ map中没有，也返回成功（因为已清理ZLM）
        }
      } catch (const std::exception &e) {
        LOG_WARN("GB28181 Gateway: 从ZLM查询流信息时出现异常: {}", e.what());
      }
    }
    LOG_WARN("GB28181 Gateway: 流不存在（Gateway streams_ "
             "map和ZLM中都未找到）: {}/{} (base: {})",
             target_app, target_stream, base_stream);
    return Result<void>::Success();
  }
  ...
  if (!info.call_id.empty()) {
    ...
    if (!device.id.empty()) {
      int device_port =
          info.device_sip_port > 0 ? info.device_sip_port : device.port;
      LOG_DEBUG("GB28181 Gateway: 发送BYE请求，使用端口: {} "
                "(StreamInfo.device_sip_port={}, device.port={})",
                device_port, info.device_sip_port, device.port);
      SendBye(info.call_id, device.ip, device_port);
    }
  ...
  if (success) {
    ...
    LOG_WARN("GB28181 Gateway 停止失败（但已清理本地记录）: {}/{}", target_app,
             target_stream);
  }
...
bool GB28181Gateway::IsRunning(const std::string &target_app,
  ...
  bool is_running = ::utils::StreamStatusChecker::CheckIsRunning(
      ...
      info.status, zlm_client_, target_app, target_stream, new_status, new_pid);
...
int GB28181Gateway::StopStreamByDevice(const std::string &device_id,
                                       const std::string &channel_id) {
  std::vector<std::pair<std::string, std::string>>
      streams_to_stop; // <app, stream>
  ...
  for (const auto &pair : streams_to_stop) {
    if (Stop(pair.first, pair.second).IsSuccess()) {
      ...
      if (last_underscore != std::string::npos) {
        std::string possible_timestamp =
            base_stream.substr(last_underscore + 1);
        bool is_timestamp = !possible_timestamp.empty() &&
                            possible_timestamp.length() >= 10 &&
                            std::all_of(possible_timestamp.begin(),
                                        possible_timestamp.end(), ::isdigit);
...
GatewayStatus GB28181Gateway::GetStatus(const std::string &target_app,
  ...
  gateway::GatewayStatus status = ::utils::StreamStatusChecker::GetStatus(
      dummy_pid, // GB28181不使用进程，pid=0
      info.status, zlm_client_, target_app, target_stream, new_pid);
...
std::vector<GB28181Device>
GB28181Gateway::DiscoverDevices(int timeout_seconds) {
...
std::string GB28181Gateway::AddDevice(const GB28181Device &device) {
  ...
  LOG_INFO("手动添加设备: {} ({}), 状态: {} (等待SIP REGISTER注册)", device.id,
           device.ip.empty() ? "N/A" : device.ip,
...
streaming::RtpServerInfo
GB28181Gateway::GetRtpServerInfo(const std::string &target_app,
...
std::string
GB28181Gateway::GetRtpStreamId(const std::string &target_app,
                               const std::string &target_stream) const {
  std::string stream_id = GenerateStreamId(target_app, target_stream);
  std::lock_guard<std::mutex> lock(streams_mutex_);

  LOG_TRACE(
      "GB28181Gateway::GetRtpStreamId: 查找 app={}, stream={}, stream_id={}",
  ...
  // 对于GB28181流，需要同时查找target_app和"live" app的流（因为前端通常使用live
  // app）
  std::vector<std::pair<std::string, std::string>>
      candidate_streams; // <rtp_stream_id, target_stream>
  LOG_TRACE("GB28181Gateway::GetRtpStreamId: 直接查找失败，收集候选流信息...");
  for (const auto &pair : streams_) {
    const StreamInfo &info = pair.second;
    // 同时查找target_app和"live" app的流（因为前端通常使用live app）
    if ((info.target_app == target_app || info.target_app == "live") &&
        !info.rtp_stream_id.empty()) {
  ...
  for (const auto &candidate : candidate_streams) {
    ...
    if (last_underscore != std::string::npos) {
      ...
      LOG_TRACE("GB28181Gateway::GetRtpStreamId: 检查流 target_stream={}, "
                "rtp_stream_id={}, rtp_without_timestamp={}",
      ...
      if (rtp_without_timestamp == target_stream) {
        LOG_TRACE(
            "GB28181Gateway::GetRtpStreamId: 匹配成功（rtp_without_timestamp "
            "== target_stream），返回 rtp_stream_id={}",
            rtp_stream_id);
    ...
    if (candidate_target_stream == target_stream) {
      LOG_TRACE("GB28181Gateway::GetRtpStreamId: 匹配成功（info.target_stream "
                "== target_stream），返回 rtp_stream_id={}",
                rtp_stream_id);
      return rtp_stream_id;
    }
  ...
  if (zlm_client_) {
      try {
          LOG_TRACE("GB28181Gateway::GetRtpStreamId:
  Gateway中未找到流信息，尝试从ZLM直接查询...");
          // 优化：使用更具体的查询，只查询rtp schema的流，减少查询时间
          auto zlm_streams = zlm_client_->GetStreamList("rtp");  // 只查询rtp
  schema的流 LOG_TRACE("GB28181Gateway::GetRtpStreamId:
  从ZLM查询到{}个RTP流，查找匹配target_stream={}的流...", zlm_streams.size(),
  target_stream);
          ...
          for (const auto& zlm_stream : zlm_streams) {
              if (zlm_stream.app == target_app &&
                  ...
                  LOG_TRACE("GB28181Gateway::GetRtpStreamId: 检查ZLM流: app={},
  stream={}, schema={}", zlm_stream.app, zlm_stream_name, zlm_stream.schema);
                  size_t last_underscore = zlm_stream_name.find_last_of('_');
                  if (last_underscore != std::string::npos) {
                      std::string zlm_stream_without_timestamp =
  zlm_stream_name.substr(0, last_underscore); if (zlm_stream_without_timestamp
  == target_stream) { LOG_TRACE("GB28181Gateway::GetRtpStreamId:
  从ZLM直接查询找到匹配的流: {}", zlm_stream_name); return zlm_stream_name;
                      }
                  }
                  // 也检查完全匹配（虽然不太可能，但为了兼容性）
                  if (zlm_stream_name == target_stream) {
                      LOG_TRACE("GB28181Gateway::GetRtpStreamId:
  从ZLM直接查询找到完全匹配的流: {}", zlm_stream_name); return zlm_stream_name;
                  }
              }
          }
          LOG_TRACE("GB28181Gateway::GetRtpStreamId:
  从ZLM直接查询也未找到匹配的流"); } catch (const std::exception& e) {
          LOG_WARN("GB28181Gateway::GetRtpStreamId: 从ZLM查询流信息时出现异常:
  {}", e.what());
      }
  }
  */
...
void GB28181Gateway::OnDeviceRegister(const std::string &device_id,
                                      ...
                                      int expires) {
  LOG_INFO("设备注册: device_id={}, ip={}, port={}, expires={}", device_id, ip,
           port, expires);
...
bool GB28181Gateway::SendInvite(const GB28181Device &device,
  ...
  if (target_ip.find("192.168.65.") == 0 && device.port == 5061) {
    ...
    LOG_DEBUG("GB28181 Gateway: 检测到Docker虚拟网络IP {}，尝试使用127.0.0.1",
              target_ip);
    target_ip = "127.0.0.1";
  }
  ...
  // 构造请求URI
  std::string uri =
      "sip:" + channel_id + "@" + target_ip + ":" + std::to_string(device.port);
  ...
  bool success = false;
  try {
    success = sip_server_->SendRequest(SipMethod::INVITE, uri, to, headers, sdp,
                                       target_ip, device.port);
  } catch (const std::exception &e) {
    LOG_ERROR("GB28181 Gateway: 发送SIP INVITE异常: device_id={}, "
              "channel_id={}, error={}",
  ...
  if (success) {
    LOG_INFO("发送SIP INVITE成功: device_id={}, channel_id={}, call_id={}, "
             "目标地址={}:{} (原始IP: {}), RTP端口={}",
             device.id, channel_id, call_id, target_ip, device.port, device.ip,
             rtp_info.port);
    LOG_DEBUG("GB28181 Gateway: INVITE请求详情 - URI={}, To={}, SDP长度={}",
              uri, to, sdp.length());
  } else {
    LOG_ERROR("GB28181 Gateway: 发送SIP INVITE失败（可能是网络异常）: "
              "device_id={}, channel_id={}, ip={}, port={}, call_id={}",
...
bool GB28181Gateway::SendBye(const std::string &call_id,
                             const std::string &device_ip, int device_port) {
  // 构造请求URI
  ...
  // 发送BYE请求
  bool success = sip_server_->SendRequest(SipMethod::BYE, uri, to,
...
std::string
GB28181Gateway::BuildSdpResponse(const streaming::RtpServerInfo &rtp_info,
...
bool GB28181Gateway::ParseSdpRequest(const std::string &sdp_body,
                                     std::string &device_ip, int &device_port) {
  // 解析SDP，提取设备的RTP接收地址
  ...
  if (!found_ip || !found_port) {
    LOG_ERROR("GB28181 Gateway: SDP解析失败 - IP: {}, Port: {}",
              found_ip ? device_ip : "未找到",
              found_port ? std::to_string(device_port) : "未找到");
    return false;
  }
...
void GB28181Gateway::CheckDeviceHeartbeat() {
  ...
  while (heartbeat_check_running_) {
    std::this_thread::sleep_for(std::chrono::seconds(
        time::GB28181_HEARTBEAT_INTERVAL_SEC)); // 心跳检查间隔
...
void GB28181Gateway::CheckStreamStatus() {
  ...
  while (stream_status_check_running_) {
    ...
    std::this_thread::sleep_for(std::chrono::seconds(
        time::GB28181_STATUS_CHECK_INTERVAL_SEC)); // 流状态检查间隔
      ...
      if (zlm_client_) {
        try {
          ...
          if (!info.rtp_stream_id.empty()) {
            ...
            stream_info = zlm_client_->GetStreamInfo(info.target_app,
                                                     info.rtp_stream_id, "rtp");
            is_active = streaming::ZLMClient::IsStreamActive(stream_info);

            // 如果找不到，尝试使用"gb28181"
            // app（兼容旧版本或如果target_app不是gb28181）
            if (!is_active && info.target_app != "gb28181") {
              stream_info = zlm_client_->GetStreamInfo(
                  "gb28181", info.rtp_stream_id, "rtp");
              is_active = streaming::ZLMClient::IsStreamActive(stream_info);
              if (is_active) {
                LOG_DEBUG("GB28181 Gateway: 在gb28181 "
                          "app中找到流（兼容旧版本）: {}/{}",
            ...
            if (is_active) {
              LOG_DEBUG(
                  "GB28181 Gateway: "
                  "使用rtp_stream_id找到活跃流（轮询检测，兜底机制）: {}/{}",
              ...
              if (info.sip_200_ok_received) {
                ...
                // 但更可能的是hook通知已经处理了，这里只是兜底检测
                LOG_DEBUG(
                    "GB28181 Gateway: 使用rtp_stream_id未找到流（已收到200 "
                    "OK）: {}/{} "
                    "(可能流还未注册到ZLM，或流名称不一致，或hook通知已处理)",
                    info.target_app, info.rtp_stream_id);
              } else {
                // 未收到200 OK，流还没注册是正常的
                LOG_DEBUG("GB28181 Gateway: "
                          "使用rtp_stream_id未找到流（未收到200 OK）: {}/{}",
                          info.target_app, info.rtp_stream_id);
              }
            }
          } else {
            // rtp_stream_id为空，使用target_stream作为后备（这种情况不应该发生）
            LOG_WARN("GB28181 Gateway: "
                     "rtp_stream_id为空，使用target_stream作为后备: {}/{}",
                     info.target_app, info.target_stream);
            stream_info = zlm_client_->GetStreamInfo(info.target_app,
                                                     info.target_stream, "rtp");
            is_active = streaming::ZLMClient::IsStreamActive(stream_info);
          }
          ...
          if (!is_active) {
            ...
            if (info.status == GatewayStatus::Starting) {
              ...
              if (!info.sip_200_ok_received) {
                ...
                if (elapsed >= stream_start_timeout_seconds_) {
                  // 超时：设备未响应200 OK
                  LOG_WARN("GB28181 Gateway: 流启动超时（{}秒未收到200 "
                           "OK响应）: {}/{} (Call-ID: {}, 设备: {}, 通道: {})",
                  ...
                  if (stream_manager_) {
                    std::string error_msg =
                        "设备未响应（超时" + std::to_string(elapsed) + "秒）";
                    using namespace gateway::utils;
                    auto [error_code, error_message] =
                        InferErrorFromMessage(error_msg);
                    stream_manager_->OnStreamCreateResult(
                        info.target_app, info.target_stream, false, error_code,
                        error_message);
                  }
                } else {
                  // 记录等待中的状态（每10秒记录一次，避免日志过多）
                  if (elapsed % 10 == 0 && elapsed > 0) {
                    LOG_DEBUG("GB28181 Gateway: 等待SIP 200 OK响应 ({}/{}秒): "
                              "{}/{} (Call-ID: {})",
                ...
                auto elapsed_since_200ok =
                    std::chrono::duration_cast<std::chrono::seconds>(
                        now - info.sip_200_ok_time)
                        .count();
                int rtp_push_timeout =
                    60; // RTP流推送超时时间（秒）-
                        // 增加到60秒，给设备更多时间启动ffmpeg
                ...
                if (elapsed_since_200ok >= rtp_push_timeout) {
                  ...
                  // 但ZLM的getRtpInfo
                  // API可能期望使用"rtp"作为app，而不是target_app
                  // 尝试两种app值，确保能正确查询
                  std::string app_to_check =
                      info.target_app.empty() ? "rtp" : info.target_app;
                    ...
                    try {
                      ...
                      try {
                        rtp_info = zlm_client_->GetRtpInfo(
                            rtp_stream_id_to_check, app_to_check,
                            "__defaultVhost__");
                        ...
                        if (app_to_check != "rtp") {
                          try {
                            rtp_info = zlm_client_->GetRtpInfo(
                                rtp_stream_id_to_check, "rtp",
                                "__defaultVhost__");
                            rtp_info_retrieved = true;
                          } catch (const std::exception &e2) {
                            LOG_WARN("GB28181 Gateway: "
                                     "查询RTP服务器状态失败（尝试了{}和rtp）: "
                                     "{}, {}",
                                     app_to_check, e1.what(), e2.what());
                          }
                        } else {
                          LOG_WARN("GB28181 Gateway: 查询RTP服务器状态失败: {}",
                                   e1.what());
                        }
                      }

                      if (rtp_info_retrieved && !rtp_info.peer_ip.empty() &&
                          rtp_info.peer_ip != "0.0.0.0") {
                        rtp_server_receiving_data = true;
                        LOG_INFO("GB28181 Gateway: "
                                 "RTP服务器已接收到数据（peer_ip={}, "
                                 "peer_port={}），但ZLM流未注册，继续等待: {}",
                                 rtp_info.peer_ip, rtp_info.peer_port,
                                 rtp_stream_id_to_check);
                      }
                    } catch (const std::exception &e) {
                      LOG_WARN(
                          "GB28181 Gateway: 查询RTP服务器状态时出现异常: {}",
                          e.what());
                    }
                  }
                  ...
                  if (rtp_server_receiving_data) {
                    LOG_DEBUG("GB28181 Gateway: "
                              "RTP服务器已接收到数据，但ZLM流未注册，保持Starti"
                              "ng状态继续等待: {}/{}",
                              info.target_app, info.target_stream);
                  } else {
                      ...
                      try {
                        ...
                        for (const auto &zlm_stream : zlm_streams) {
                          if (zlm_stream.app == target_app_to_check &&
                              ...
                              streaming::ZLMClient::IsStreamActive(
                                  zlm_stream)) {
                            stream_found_in_zlm = true;
                            LOG_INFO("GB28181 Gateway: "
                                     "在ZLM媒体列表中找到了流（之前GetStreamInf"
                                     "o未找到）: {}/{}",
                                     target_app_to_check,
                                     rtp_stream_id_to_check);
                            break;
                        ...
                        LOG_WARN(
                            "GB28181 Gateway: 查询ZLM媒体列表时出现异常: {}",
                            e.what());
                      }
                    }
                    ...
                    if (stream_found_in_zlm) {
                      ...
                      LOG_DEBUG("GB28181 Gateway: "
                                "流在ZLM中运行，但GetStreamInfo未找到，保持Star"
                                "ting状态等待hook通知: {}/{}",
                                info.target_app, info.target_stream);
                    } else {
                      // 确认流确实不存在且RTP服务器未接收到数据，标记为错误并清理
                      LOG_WARN("GB28181 Gateway: RTP流推送超时（收到200 "
                               "OK后{}"
                               "秒仍未在ZLM中检测到流，且RTP服务器未接收到数据"
                               "）: {}/{} (Call-ID: {}, 设备: {}, 通道: {})",
                               elapsed_since_200ok, info.target_app,
                               info.target_stream,
                               info.call_id.empty() ? "N/A" : info.call_id,
                               info.device_id, info.channel_id);
                      info.status = GatewayStatus::Error;
                      ...
                      if (stream_manager_) {
                        std::string error_msg =
                            "RTP流推送超时（收到200 OK后" +
                            std::to_string(elapsed_since_200ok) +
                            "秒仍未检测到流）";
                        using namespace gateway::utils;
                        auto [error_code, error_message] =
                            InferErrorFromMessage(error_msg);
                  ...
                  if (elapsed_since_200ok % 5 == 0 && elapsed_since_200ok > 0) {
                    LOG_DEBUG(
                        "GB28181 Gateway: 已收到200 OK，等待RTP流推送到ZLM "
                        "({}/{}秒): {}/{} (Call-ID: {})",
                        elapsed_since_200ok, rtp_push_timeout, info.target_app,
                        info.target_stream,
                        info.call_id.empty() ? "N/A" : info.call_id);
                  }
                }
            ...
            if (info.status == GatewayStatus::Starting) {
              ...
              if (info.sip_200_ok_received) {
                ...
                LOG_INFO(
                    "GB28181 Gateway: "
                    "流状态验证成功（通过轮询检测，兜底机制），流已完全启动: "
                    "{}/{} (总耗时: {}ms, 收到200 OK后RTP推送耗时: {}ms)",
                    info.target_app, info.target_stream, total_elapsed,
                    rtp_elapsed);
              } else {
                LOG_INFO("GB28181 Gateway: "
                         "流状态验证成功（通过轮询检测，兜底机制），流已完全启"
                         "动: {}/{} (总耗时: {}ms)",
                         info.target_app, info.target_stream, total_elapsed);
              }
              ...
              if (stream_manager_) {
                GatewayBase::RegisterStreamToManager(
                    stream_manager_, info.target_app, info.target_stream,
                    "gb28181", info.output_protocol, info.source_url,
                    "gb28181_gateway", GatewayStatus::Running,
                    0, // 不使用FFmpeg进程
                    info.device_id, "gb28181");

                stream_manager_->OnStreamCreateResult(info.target_app,
                                                      info.target_stream, true);
              }
            }
          }
        ...
        if (it != streams_.end()) {
          ...
          if (zlm_client_ && !it->second.rtp_stream_id.empty()) {
            try {
              ...
              LOG_INFO("GB28181 Gateway 已关闭失败的RTP服务器: {}",
                       it->second.rtp_stream_id);
            } catch (const std::exception &e) {
    ...
    if (inet_aton("8.8.8.8", &addr.sin_addr) == 1) {
      if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
        ...
        if (getsockname(sock, (struct sockaddr *)&local_addr, &len) == 0) {
          ...
          if (inet_ntop(AF_INET, &local_addr.sin_addr, ip_str,
...
void GB28181Gateway::OnSipResponse(const std::string &call_id, int status_code,
                                   const std::string &reason_phrase,
                                   int peer_port) {
  LOG_INFO("GB28181 Gateway: 收到SIP响应: Call-ID={}, status={} {}", call_id,
           status_code, reason_phrase);

  // 通过call_id查找对应的流
  // 优化：先收集候选流信息，然后释放锁再进行比较，减少锁持有时间
  std::vector<std::pair<std::string, std::string>>
      candidate_streams; // <stream_id, call_id>
  {
    std::lock_guard<std::mutex> lock(streams_mutex_);
    for (const auto &pair : streams_) {
  ...
  if (stream_id.empty()) {
    LOG_WARN("GB28181 Gateway: 收到响应但未找到对应的流: Call-ID={}, status={} "
             "{} (可能Call-ID不匹配或流已清理)",
  ...
  if (status_code == 200) {
    ...
    if (info.status == GatewayStatus::Starting) {
      ...
      LOG_INFO("GB28181 Gateway: 收到200 OK响应（SIP信令成功）: {}/{} (耗时: "
               "{}ms, Call-ID: {}, 设备端口: {}), 等待RTP流推送到ZLM...",
               info.target_app, info.target_stream, elapsed, call_id,
               info.device_sip_port > 0 ? std::to_string(info.device_sip_port)
                                        : "未提取");

      // 重要：不立即将状态设为Running，等待ZLM的hook通知或轮询检测到RTP流活跃
      // 更推荐的方式是依赖ZLM的on_stream_changed hook通知，因为hook通知是实时的
      // 当ZLM检测到RTP流注册时，会调用on_stream_changed
      // hook，StreamManager会收到通知
    ...
    LOG_WARN(
        "GB28181 Gateway: 收到错误响应: Call-ID={}, status={} {}, stream={}/{}",
        call_id, status_code, reason_phrase, info.target_app,
        info.target_stream);
    ...
    if (stream_manager_) {
      std::string error_msg =
          "设备响应错误: " + std::to_string(status_code) + " " + reason_phrase;
      using namespace gateway::utils;
      auto [error_code, error_message] = InferErrorFromMessage(error_msg);
      stream_manager_->OnStreamCreateResult(info.target_app, info.target_stream,
...
bool GB28181Gateway::OnByeRequest(const std::string &call_id) {
  ...
  if (stream_id.empty()) {
    LOG_WARN("GB28181 Gateway: 收到BYE请求但未找到对应的流: Call-ID={}",
             call_id);
    return false;
  }
  ...
  LOG_INFO("GB28181 Gateway: BYE请求处理成功，已停止流: {}/{}", info.target_app,
           info.target_stream);
  return true;
}
