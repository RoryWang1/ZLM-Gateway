...
namespace {
static gateway::utils::AutoRegister reg("dahua", [](const gateway::utils::GatewayContext& ctx) -> std::shared_ptr<gateway::GatewayBase> {
    if (ctx.config->dahua.enabled) {
        return std::make_shared<DahuaGateway>(ctx.config, ctx.zlm_client, ctx.process_manager, ctx.stream_manager);
    }
    return nullptr;
});
}

DahuaGateway::DahuaGateway(std::shared_ptr<config::Config> config,
    ...
    LOG_INFO("Dahua Gateway 初始化完成（支持直接代理和转码）");
}

DahuaGateway::~DahuaGateway() {
    // 停止所有流
...
Result<void> DahuaGateway::Start(const std::string& source_url,
                        const std::string& target_app,
                        const std::string& target_stream,
                        const std::string& /* output_protocol */) {
    // source_url 格式: dahua://device_id[/channel_id]
    // 例如: dahua://device_123 或 dahua://device_123/1
    
    if (!::utils::url_parser::ValidateURL(source_url, "dahua://")) {
        return Result<void>::Failure(gateway::InvalidParameterException("Invalid source URL format: " + source_url));
    }
    
    auto [device_id, channel_id] = ::utils::url_parser::ParseDeviceURL(source_url, "dahua://");
    
    return StartDeviceStream(device_id, channel_id, target_app, target_stream);
}

Result<void> DahuaGateway::Stop(const std::string& target_app,
                       const std::string& target_stream) {
    return StopDeviceStream(target_app, target_stream);
}

bool DahuaGateway::IsRunning(const std::string& target_app,
                            const std::string& target_stream) {
...
GatewayStatus DahuaGateway::GetStatus(const std::string& target_app,
                                     const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
    std::lock_guard<std::mutex> lock(streams_mutex_);
    auto it = streams_.find(stream_id);
...
std::vector<DahuaDevice> DahuaGateway::DiscoverDevices(const std::string& ip_range,
                                                       int port,
                                                       int timeout_seconds) {
    LOG_INFO("开始发现 Dahua 设备，超时时间: {} 秒", timeout_seconds);
    ...
    LOG_INFO("发现 {} 个 Dahua 设备，已保存到设备列表", devices.size());
    return devices;
}

std::vector<DahuaDevice> DahuaGateway::ListDevices() const {
    std::lock_guard<std::mutex> lock(devices_mutex_);
    
    std::vector<DahuaDevice> result;
...
DahuaDevice DahuaGateway::GetDevice(const std::string& device_id) const {
    std::lock_guard<std::mutex> lock(devices_mutex_);
    
    auto it = devices_.find(device_id);
    if (it == devices_.end()) {
        return DahuaDevice();  // 返回空对象
    }
    
    return it->second;
}

std::string DahuaGateway::AddDevice(const DahuaDevice& device) {
    std::lock_guard<std::mutex> lock(devices_mutex_);
    ...
    DahuaDevice device_copy = device;
...
bool DahuaGateway::RemoveDevice(const std::string& device_id) {
    std::lock_guard<std::mutex> lock(devices_mutex_);
...
bool DahuaGateway::UpdateDeviceCredentials(const std::string& device_id,
...
std::vector<std::string> DahuaGateway::GetDeviceRTSPURLs(const std::string& device_id,
                                                         const std::string& channel_id) {
    DahuaDevice device = GetDevice(device_id);
    if (device.id.empty()) {
        LOG_ERROR("设备不存在: {}", device_id);
        return {};
    }
    ...
    if (channel_id.empty()) {
        // 获取所有通道的 RTSP 地址
        std::vector<std::string> channels = GetDeviceChannels(device.base_url, device.username, device.password);
        for (const auto& ch_id : channels) {
            std::string url = GetStreamUri(device.base_url, ch_id, device.username, device.password);
            if (!url.empty()) {
        ...
        // 获取指定通道的 RTSP 地址
        std::string url = GetStreamUri(device.base_url, channel_id, device.username, device.password);
        if (!url.empty()) {
            rtsp_urls.push_back(url);
        }
    }
...
Result<void> DahuaGateway::StartDeviceStream(const std::string& device_id,
                                    const std::string& channel_id,
                                    const std::string& target_app,
                                    const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
    ...
    auto result = smart_processor_->ProcessStream(
        rtsp_url, target_app, target_stream, "", "rtsp", "dahua_gateway",
        // 直接代理回调
        ...
        [this, &info, stream_id](const gateway::utils::StreamInfoResult& stream_info_result) {
            ...
            int recommended_bitrate = bitrate_helper_->GetRecommendedBitrate(
                info.target_app, info.target_stream, "dahua", "",
            ...
            LOG_DEBUG("[Dahua Gateway] FFmpeg 命令: {}", command);
            
            int pid = 0;
            GatewayStatus status = GatewayStatus::Stopped;
            
            std::string log_file = "/tmp/ffmpeg_dahua_" + info.target_app + "_" + info.target_stream + ".log";
    ...
    if (result.success) {
        ...
        LOG_INFO("Dahua Gateway 启动成功: 设备 {} -> {}/{}", device_id, target_app, target_stream);
        return Result<void>::Success();
    } else {
...
Result<void> DahuaGateway::StopDeviceStream(const std::string& target_app,
                                   const std::string& target_stream) {
    std::string stream_id = GenerateStreamId(target_app, target_stream);
...
std::string DahuaGateway::BuildFFmpegCommand(const StreamInfo& info, 
    ...
    if (video_only_transcode) {
        ...
        LOG_INFO("[DahuaGateway] Video({}) is compatible, transcoding audio only, using -c:v copy", 
    ...
    if (stream_info_result.audio_codec == "aac") {
        ...
        LOG_INFO("[DahuaGateway] 源流音频是 AAC，使用 -c:a copy 保持质量");
    } else {
        oss << " -c:a aac -b:a 128k -ar 44100 -ac 2";  // 只有不是 AAC 时才转码
        LOG_INFO("[DahuaGateway] 源流音频是 {}，转码为 AAC", stream_info_result.audio_codec.empty() ? "(unknown)" : stream_info_result.audio_codec);
    }
    
    // 输出参数 - 统一使用 RTMP/FLV 推流格式
    oss << " -f flv"                    // 输出格式为 FLV (推送到 ZLM)
        << " \"" << ::utils::zlm_url_builder::BuildRTMPUrlWithSecret(
            config_, info.target_app, info.target_stream, "Dahua Gateway") << "\"";
    
    return oss.str();
}


std::vector<DahuaDevice> DahuaGateway::DiscoverDevicesHTTP(const std::string& ip_range,
                                                           int port,
                                                           int /* timeout_seconds */) {
    std::vector<DahuaDevice> devices;
    ...
    for (const auto& ip : ip_list) {
        DahuaDevice device = GetDeviceInfo("http://" + ip + ":" + std::to_string(port), "", "");
        if (!device.id.empty()) {
...
std::string DahuaGateway::SendHTTPRequest(const std::string& url,
    ...
    try {
        ...
        if (res != CURLE_OK) {
            LOG_ERROR("Dahua 请求失败: {} (CURL error: {})", url, curl_easy_strerror(res));
            return "";
        }
        
        if (response_code != 200) {
            LOG_ERROR("Dahua 请求返回错误状态码: {} (HTTP {})", url, response_code);
            return "";
        }
        
        LOG_DEBUG("Dahua 请求成功: {} (HTTP {}), 响应长度: {} 字节", url, response_code, response_data.length());
        return response_data;
    } catch (const std::exception& e) {
        LOG_ERROR("Dahua 请求异常: {} (异常信息: {})", url, e.what());
        curl_slist_free_all(headers);
        curl_easy_cleanup(curl);
        return "";
    } catch (...) {
        LOG_ERROR("Dahua 请求未知异常: {}", url);
        curl_slist_free_all(headers);
        curl_easy_cleanup(curl);
        return "";
    }
...
DahuaDevice DahuaGateway::GetDeviceInfo(const std::string& base_url,
                                        const std::string& username,
                                        const std::string& password) {
    // 大华设备信息 API: /cgi-bin/global.cgi?action=getDeviceInfo
    std::string url = base_url + "/cgi-bin/global.cgi?action=getDeviceInfo";
    std::string response = SendHTTPRequest(url, "GET", "", username, password);
    ...
    // 解析 JSON 或 XML 响应（大华设备可能返回 JSON 或 XML）
    // 示例响应格式（JSON）：
    // {"deviceName":"Dahua","model":"IPC-HDW1234","serialNumber":"...","firmwareVersion":"..."}
    
    std::regex model_regex(R"delim("model"\s*:\s*"([^"]+)")delim");
    std::regex serial_regex(R"delim("serialNumber"\s*:\s*"([^"]+)")delim");
    std::regex version_regex(R"delim("firmwareVersion"\s*:\s*"([^"]+)")delim");
...
std::vector<std::string> DahuaGateway::GetDeviceChannels(const std::string& base_url,
                                                         const std::string& username,
                                                         const std::string& password) {
    // 大华通道列表 API: /cgi-bin/global.cgi?action=getChannels
    std::string url = base_url + "/cgi-bin/global.cgi?action=getChannels";
    std::string response = SendHTTPRequest(url, "GET", "", username, password);
    ...
    // 解析 JSON 响应，提取通道 ID
    // 示例: {"channels":[{"id":1,"name":"Channel 1"},{"id":2,"name":"Channel 2"}]}
    std::regex channel_regex(R"("id"\s*:\s*(\d+))");
...
std::string DahuaGateway::GetStreamUri(const std::string& base_url,
                                       const std::string& channel_id,
                                       const std::string& username,
                                       const std::string& password) {
    // 优先尝试从 API 获取 RTSP URL（如果设备支持）
    std::string url = base_url + "/cgi-bin/global.cgi?action=getStreamUrl&channel_id=" + channel_id;
    std::string response = SendHTTPRequest(url, "GET", "", username, password);
    
    if (!response.empty()) {
        // 解析 JSON 响应，提取 RTSP URL
        // 示例: {"url":"rtsp://127.0.0.1:8554/test_stream","channel_id":"1","format":"rtsp"}
        std::regex url_regex(R"delim("url"\s*:\s*"([^"]+)")delim");
        std::smatch match;
        
        if (std::regex_search(response, match, url_regex)) {
            std::string rtsp_url = match[1].str();
            LOG_DEBUG("从 API 获取 RTSP URL: {}", rtsp_url);
            return rtsp_url;
        }
    }
    
    // 如果 API 获取失败，构建默认 RTSP URL
    // 大华设备 RTSP 流地址格式: rtsp://ip:554/cam/realmonitor?channel=1&subtype=0
    size_t pos = base_url.find("://");
    if (pos == std::string::npos) {
        return "";
    }
    
    std::string ip_port = base_url.substr(pos + 3);
    size_t colon_pos = ip_port.find(':');
    std::string ip = (colon_pos != std::string::npos) ? ip_port.substr(0, colon_pos) : ip_port;
    
    // 检测是否是模拟器（127.0.0.1:8083），如果是则使用 8554 端口的模拟流
    if (ip == "127.0.0.1" && base_url.find(":8083") != std::string::npos) {
        // 模拟器：使用 8554 端口的模拟流（与其他协议一致）
        std::string rtsp_url = "rtsp://127.0.0.1:8554/test_stream";
        LOG_DEBUG("检测到模拟器，使用模拟 RTSP URL: {}", rtsp_url);
        return rtsp_url;
    }
    
    // 真实设备：使用标准大华 RTSP URL 格式
    std::string rtsp_url = "rtsp://" + ip + ":554/cam/realmonitor?channel=" + channel_id + "&subtype=0";
    if (!username.empty()) {
        rtsp_url = "rtsp://" + username + ":" + password + "@" + ip + ":554/cam/realmonitor?channel=" + channel_id + "&subtype=0";
    }
    
    return rtsp_url;
}

std::string DahuaGateway::GenerateDeviceID(const std::string& base_url) const {
    std::hash<std::string> hasher;
    size_t hash = hasher(base_url);
    std::ostringstream oss;
    oss << "dahua_device_" << std::hex << hash;
    return oss.str();
}

std::string DahuaGateway::GenerateUUID() const {
    // 简单实现：使用时间戳和随机数生成 UUID
    auto now = std::chrono::system_clock::now();
...
std::vector<std::string> DahuaGateway::ParseIPRange(const std::string& ip_range) const {
