#ifndef GATEWAY_DAHUA_DAHUA_GATEWAY_HPP
#define GATEWAY_DAHUA_DAHUA_GATEWAY_HPP
...
struct DahuaDevice {
    ...
    std::string base_url;                // 设备基础 URL (e.g., http://ip:port)
    std::string manufacturer;            // 制造商（通常为 "Dahua"）
    std::string model;                   // 型号
 ...
 *
 * 功能：
 * 1. 发现大华设备（通过 HTTP API 扫描或手动添加）
 * 2. 调用大华 API 获取设备的 RTSP 流地址
 * 3. 管理设备的流（直接推流到 ZLMediaKit，因为 RTSP 是原生协议）
 */
class DahuaGateway : public GatewayBase {
public:
    /**
     * @brief 构造函数
     * @param config 配置对象
     ...
     */
    DahuaGateway(std::shared_ptr<config::Config> config,
    ...
    std::string GetProtocol() const override { return "dahua"; }

    /**
     * @brief 发现大华设备
     * @param ip_range IP 地址范围（CIDR 格式，如 "192.168.1.0/24"），空则扫描本地网络
     * @param timeout_seconds 超时时间（秒），默认 5 秒
     * @return 发现的设备列表
     */
    std::vector<DahuaDevice> DiscoverDevices(const std::string& ip_range = "", int port = 80, int timeout_seconds = 5);

    /**
     * @brief 获取所有设备列表
     * @return 设备列表
     */
    std::vector<DahuaDevice> ListDevices() const;
     ...
     */
    DahuaDevice GetDevice(const std::string& device_id) const;
     ...
     * @return 设备 ID
     */
    std::string AddDevice(const DahuaDevice& device);
     ...
     * @brief 获取设备的 RTSP 地址
     * @param device_id 设备 ID
     * @param channel_id 通道 ID（可选，默认获取第一个通道）
     * @return RTSP 流地址列表
     */
...
private:
    ...
    std::map<std::string, DahuaDevice> devices_; // key: device_id
    mutable std::mutex devices_mutex_;
    std::map<std::string, StreamInfo> streams_; // key: app/stream
     ...
     * @return 发现的设备列表
     */
    std::vector<DahuaDevice> DiscoverDevicesHTTP(const std::string& ip_range,
                                                  int port,
                                                  int timeout_seconds);
    ...
    /**
     * @brief 获取设备信息 (大华 /cgi-bin/global.cgi?action=getDeviceInfo)
     * @param base_url 设备基础 URL
     * @param username 用户名
     * @param password 密码
     * @return 设备信息
     */
    DahuaDevice GetDeviceInfo(const std::string& base_url,
                              const std::string& username,
                              const std::string& password);

    /**
     * @brief 获取设备通道列表 (大华 /cgi-bin/global.cgi?action=getChannels)
     * @param base_url 设备基础 URL
     * @param username 用户名
     * @param password 密码
     * @return 通道 ID 列表
     */
    std::vector<std::string> GetDeviceChannels(const std::string& base_url,
                                               const std::string& username,
                                               const std::string& password);

    /**
     * @brief 获取 RTSP 流 URI (大华设备通常使用固定格式)
     * @param base_url 设备基础 URL
     * @param channel_id 通道 ID
     * @param username 用户名
     * @param password 密码
     * @return RTSP URI
     */
    std::string GetStreamUri(const std::string& base_url,
                             const std::string& channel_id,
                             const std::string& username,
                             const std::string& password);
...
#endif // GATEWAY_DAHUA_DAHUA_GATEWAY_HPP
