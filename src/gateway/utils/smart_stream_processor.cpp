...
StreamProcessResult SmartStreamProcessor::ProcessStream(
    const std::string &source_url, const std::string &target_app,
    const std::string &target_stream, const std::string &output_protocol,
    const std::string &protocol, const std::string &gateway_type,
    ...
    const std::string &stream_schema, int detect_timeout,
    const std::string &detect_username, const std::string &detect_password) {
  ...
  // 2. 检测流信息
  result.stream_info = DetectStreamInfo(source_url, detect_timeout,
                                        detect_username, detect_password);
  ...
  if (result.stream_info.valid) {
    ...
    LOG_INFO(
        "[SmartStreamProcessor] Detected stream info - Audio: {} ({}Hz, {}ch, "
        "{}kbps), Video: {} ({}x{}@{}fps, {}kbps), Total: {}kbps",
        result.stream_info.audio_codec.empty() ? "(none)"
                                               : result.stream_info.audio_codec,
        result.stream_info.audio_sample_rate > 0
            ? result.stream_info.audio_sample_rate
            : 0,
        result.stream_info.audio_channels > 0
            ? result.stream_info.audio_channels
            : 0,
        result.stream_info.audio_bitrate > 0
            ? result.stream_info.audio_bitrate / 1000
            : 0,
        result.stream_info.video_codec.empty() ? "(none)"
                                               : result.stream_info.video_codec,
        ...
        result.stream_info.video_bitrate > 0
            ? result.stream_info.video_bitrate / 1000
            : 0,
        result.stream_info.total_bitrate > 0
            ? result.stream_info.total_bitrate / 1000
            : 0);
  } else {
    LOG_WARN("[SmartStreamProcessor] Stream info detection failed, will try "
             "direct proxy first");
  }
  ...
  if (!use_ffmpeg) {
    ...
    DirectProxyResult proxy_result =
        TryDirectProxy(target_app, target_stream, source_url,
    ...
    if (proxy_result == DirectProxyResult::Success) {
      ...
      LOG_WARN("[SmartStreamProcessor] Direct proxy failed with permanent "
               "error ({}: {}), falling back to FFmpeg transcoding",
               proxy_error_code, proxy_error_message);
      use_ffmpeg = true;
      ...
      if (protocol == "http-flv" || protocol == "hls") {
        LOG_WARN(
            "[SmartStreamProcessor] Direct proxy API returned timeout for {} "
            "stream ({}: {}), but checking if stream was actually created",
            protocol, proxy_error_code, proxy_error_message);

        // 等待一段时间，然后检查流是否真的创建了
        LOG_INFO("[SmartStreamProcessor] Waiting {} seconds before checking if "
                 "stream was created...",
                 time::SMART_STREAM_FALLBACK_WAIT_MS / 1000);
        std::this_thread::sleep_for(
            std::chrono::milliseconds(time::SMART_STREAM_FALLBACK_WAIT_MS));

        auto check_stream_info = get_stream_info_callback(
            target_app, target_stream,
            stream_schema.empty() ? protocol : stream_schema);
        LOG_INFO("[SmartStreamProcessor] Checked stream info: app={}, "
                 "stream={}, alive={}, speed={} bytes/s, total={} bytes",
                 check_stream_info.app, check_stream_info.stream,
                 check_stream_info.alive, check_stream_info.bytes_speed,
                 check_stream_info.total_bytes);

        if (check_stream_info.app == target_app &&
            check_stream_info.stream == target_stream) {
          // 流已创建！继续等待数据传输（给更多时间）
          LOG_INFO("[SmartStreamProcessor] ✅ Stream {}/{} was created despite "
                   "API timeout, waiting for data transfer...",
                   target_app, target_stream);

          // 再等待一段时间，检查是否有数据传输（增加到 30 次，每次 2 秒，总共
          // 60 秒）
          for (int wait_retry = 0; wait_retry < 30; ++wait_retry) {
            std::this_thread::sleep_for(std::chrono::milliseconds(
                time::SMART_STREAM_RETRY_INTERVAL_MS));
            auto wait_stream_info = get_stream_info_callback(
                target_app, target_stream,
                stream_schema.empty() ? protocol : stream_schema);

            if (wait_retry % 5 == 0) {
              LOG_INFO(
                  "[SmartStreamProcessor] Waiting for data transfer: retry "
                  "{}/30, alive={}, speed={} bytes/s, total={} bytes",
                  wait_retry + 1, wait_stream_info.alive,
                  wait_stream_info.bytes_speed, wait_stream_info.total_bytes);
            }

            if (streaming::ZLMClient::IsStreamActive(wait_stream_info)) {
              // 流已活跃，直接代理成功！
              LOG_INFO("[SmartStreamProcessor] ✅✅✅ Direct proxy successful "
                       "after API timeout: {}/{} (alive={}, speed={} bytes/s, "
                       "total={} bytes, waited {} seconds)",
                       target_app, target_stream, wait_stream_info.alive,
                       wait_stream_info.bytes_speed,
          ...
          LOG_WARN("[SmartStreamProcessor] ❌ Stream {}/{} was created but no "
                   "data transfer after 60 seconds, falling back to FFmpeg",
                   target_app, target_stream);
          if (zlm_client_) {
            zlm_client_->DeleteStream(target_app, target_stream);
          }
        } else {
          LOG_WARN("[SmartStreamProcessor] Stream {}/{} was not created in ZLM "
                   "after API timeout",
                   target_app, target_stream);
        }
      }
      ...
      // 回退到 FFmpeg 转码
      LOG_WARN("[SmartStreamProcessor] Direct proxy failed with transient "
               "error ({}: {}), falling back to FFmpeg transcoding",
               proxy_error_code, proxy_error_message);
      use_ffmpeg = true;
      result.use_ffmpeg = true;
  ...
  if (use_ffmpeg) {
    bool ffmpeg_success = StartFFmpegTranscode(
        target_app, target_stream, result.stream_info, start_ffmpeg_callback,
        read_ffmpeg_error_callback, get_stream_info_callback, result.pid,
        result.status);
...
void SmartStreamProcessor::RegisterStream(
...
void SmartStreamProcessor::ReportStreamResult(
    const std::string &target_app, const std::string &target_stream,
    bool success, const std::string &error_code,
    const std::string &error_message) {
  if (stream_manager_) {
    stream_manager_->OnStreamCreateResult(target_app, target_stream, success,
                                          error_code, error_message);
  }
}

StreamInfoResult SmartStreamProcessor::DetectStreamInfo(
    const std::string &source_url, int timeout_seconds,
  ...
  if (!username.empty() && !password.empty()) {
    return stream_info_detector_->Detect(source_url, username, password,
                                         timeout_seconds);
  } else {
...
void SmartStreamProcessor::MakeTranscodingDecision(
  ...
  if (stream_info_result.valid) {
    ...
    bool video_compatible = StreamInfoDetector::IsVideoCodecCompatible(
        stream_info_result.video_codec);
    bool audio_compatible = StreamInfoDetector::IsAudioCodecCompatible(
...
SmartStreamProcessor::DirectProxyResult SmartStreamProcessor::TryDirectProxy(
  ...
  // 调用直接代理回调
  LOG_INFO(
      "[SmartStreamProcessor] Calling direct proxy callback for {}/{} -> {}",
      target_app, target_stream, source_url);
  bool callback_result =
      direct_proxy_callback(target_app, target_stream, source_url);
  LOG_INFO("[SmartStreamProcessor] Direct proxy callback returned: {}",
           callback_result ? "true (success)" : "false (failed)");
  ...
  if (!callback_result) {
    ...
    if (lower_url.find("rtsp://") == 0 &&
        lower_url.find("@") != std::string::npos) {
      // RTSP URL 包含认证信息，可能是认证失败
      error_code = ErrorCode::SOURCE_AUTH_FAILED;
      error_message =
          ErrorMessageMapper::GetMessage(ErrorCode::SOURCE_AUTH_FAILED);
    }

    // 对于 HTTP-FLV 等长连接流，即使回调返回
    // false，也可能是超时（流可能已创建） 这里返回 Timeout，让上层继续检查
    if (lower_url.find("http://") == 0 || lower_url.find("https://") == 0) {
      LOG_WARN("[SmartStreamProcessor] Direct proxy callback failed for "
               "HTTP/HTTPS stream {}/{}: {} ({}), but may be timeout - will "
  ...
  for (int retry = 0; retry < max_retries_; ++retry) {
    ...
    auto stream_info =
        get_stream_info_callback(target_app, target_stream, stream_schema);

    // 使用统一的流活跃性判断函数
    if (streaming::ZLMClient::IsStreamActive(stream_info)) {
      // 流已成功建立且有数据传输
      LOG_INFO(
          "[SmartStreamProcessor] Direct proxy successful, stream is active: "
          "{}/{} (alive={}, speed={} bytes/s, total={} bytes, waited {}ms)",
    ...
    if (retry < max_retries_ - 1) {
      LOG_DEBUG("[SmartStreamProcessor] Direct proxy stream not yet active, "
                "waiting for retry {}/{}: {}/{} (alive={}, speed={} bytes/s)",
  ...
  if (final_stream_info.app == target_app &&
      final_stream_info.stream == target_stream) {
    // 流已注册但无数据传输
    // 对于 HTTP-FLV
    // 长连接流，即使已注册，也可能需要等待一段时间才能开始传输数据
    // 如果流已注册（说明 ZLM
    // 已连接到源流），即使暂时没有数据，也不应该立即判定为失败
    ...
    LOG_WARN("[SmartStreamProcessor] Direct proxy created but stream not "
             "active (no data transfer in {} seconds), may need more time for "
             "long-lived streams: {}/{} (alive={}, speed={} bytes/s)",
             max_retries_ * retry_interval_ms_ / 1000, target_app,
             target_stream, final_stream_info.alive,
             final_stream_info.bytes_speed);
    ...
    error_code = ErrorCode::STREAM_TIMEOUT;
    error_message = ErrorMessageMapper::GetMessage(ErrorCode::STREAM_TIMEOUT);
    LOG_WARN("[SmartStreamProcessor] Direct proxy stream not registered after "
             "{} seconds, timeout: {}/{}",
             max_retries_ * retry_interval_ms_ / 1000, target_app,
             target_stream);
...
bool SmartStreamProcessor::StartFFmpegTranscode(
    const std::string &target_app, const std::string &target_stream,
    const StreamInfoResult &stream_info_result,
    StartFFmpegCallback start_ffmpeg_callback,
    ReadFFmpegErrorCallback read_ffmpeg_error_callback,
    GetStreamInfoCallback get_stream_info_callback, int &pid,
    GatewayStatus &status) {
  LOG_INFO("[SmartStreamProcessor] ▶️  StartFFmpegTranscode called for {}/{}",
           target_app, target_stream);

  if (!stream_start_validator_) {
    // 如果没有验证器，直接调用回调
    LOG_INFO("[SmartStreamProcessor] No validator, calling FFmpeg callback "
             "directly...");
  ...
  LOG_INFO("[SmartStreamProcessor] Using StreamStartValidator to validate "
           "FFmpeg start");
  // 使用 StreamStartValidator 验证
  // 注意：StreamStartValidator 的 ValidateSimple 方法需要更新以支持
  // StreamInfoResult 暂时使用包装函数
  auto wrapped_callback = [this, start_ffmpeg_callback, stream_info_result,
                           target_app, target_stream]() {
    LOG_INFO("[SmartStreamProcessor] Wrapped callback invoked, calling actual "
             "FFmpeg start...");
    pid_t result = start_ffmpeg_callback(stream_info_result);
    LOG_INFO("[SmartStreamProcessor] Wrapped callback returned PID: {}",
             result);

    // Critical Fix: Register PID in StreamManager immediately!
    // This ensures StreamManager knows the PID during the validation phase,
    // allowing it to protect the stream from being marked as Stopped by
    // SyncZLMStatus if the ZLM API temporarily returns false negatives.
    if (result > 0 && stream_manager_) {
      streaming::StreamMetadata meta;
      meta.pid = result;
      stream_manager_->UpdateStreamMetadata(target_app, target_stream, meta);
      LOG_INFO(
          "[SmartStreamProcessor] Early PID registration for {}/{}: PID={}",
          target_app, target_stream, result);
    }
  ...
  bool success = stream_start_validator_->ValidateSimple(
      pid, status, target_app, target_stream, wrapped_callback,
      read_ffmpeg_error_callback,
      [get_stream_info_callback](const std::string &app,
                                 const std::string &stream,
                                 const std::string &schema) {
        return get_stream_info_callback(app, stream, schema);
      });

  LOG_INFO("[SmartStreamProcessor] ValidateSimple returned: {} (PID: {}, "
           "Status: {})",
