 ...
 * @return 流信息
 */
using GetStreamInfoCallback = std::function<streaming::StreamInfo(const std::string& target_app,
                                                                   const std::string& target_stream,
 ...
 * @return 进程ID，失败返回0
 */
using StartFFmpegCallback = std::function<int(const StreamInfoResult& stream_info_result)>;
...
public:
     ...
     * @param protocol 输入协议（rtsp, http-flv, hls, rtmp 等，用于 StreamManager 注册）
...
private:
     ...
     * @return 直接代理结果
     */
    DirectProxyResult TryDirectProxy(const std::string& target_app,
                                    const std::string& target_stream,
                                    const std::string& source_url,
                                    DirectProxyCallback direct_proxy_callback,
                                    GetStreamInfoCallback get_stream_info_callback,
