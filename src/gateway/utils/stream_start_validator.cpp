...
bool StreamStartValidator::ValidateWithProcessManager(
    ...
    if (!process_manager_) {
        LOG_ERROR("[StreamStartValidator] ProcessManager is null when ValidateWithProcessManager is called");
    ...
    if (!start_process_callback || !start_process_callback()) {
        LOG_ERROR("[StreamStartValidator] ProcessManager::StartProcess returned false");
        status = GatewayStatus::Error;
        pid = 0;
        return false;
    }
    ...
    // 等待进程启动稳定
    std::this_thread::sleep_for(std::chrono::milliseconds(config_.process_stable_wait_ms));
    ...
    // 等待 ZLM 中流就绪
    return WaitForStreamReadyWithProcessManager(
        pid, status, process_id, target_app, target_stream, read_error_log_callback);
}
...
bool StreamStartValidator::ValidateSimple(
    ...
    // 等待 ZLM 中流就绪
    return WaitForStreamReadySimple(
        pid, status, target_app, target_stream, read_error_log_callback);
}
...
bool StreamStartValidator::WaitForStreamReadyWithProcessManager(
    ...
    std::function<std::string()> read_error_log_callback) {
    if (!process_manager_) {
        LOG_ERROR("[StreamStartValidator] ProcessManager is null in WaitForStreamReadyWithProcessManager");
        status = GatewayStatus::Error;
        pid = 0;
        return false;
    }
    ...
    for (int attempt = 0;
         attempt < config_.max_check_attempts &&
        ...
        if (!process::ProcessMonitor::IsProcessAlive(process_info.pid)) {
            LOG_ERROR("[StreamStartValidator] FFmpeg 进程已退出，流启动失败: {}/{} (等待时间: {}ms)",
        ...
        if (CheckZLMStream(target_app, target_stream)) {
            ...
            LOG_INFO("[StreamStartValidator] 流已成功推送到 ZLM: {}/{} (等待时间: {}ms)",
                     target_app, target_stream, total_wait_time);
            return true;
        }
        ...
        if (attempt == 0 && config_.max_check_attempts > 1) {
            ...
            check_interval = std::min(remaining_timeout, config_.zlm_check_interval_ms * 2);
        }
    }
    ...
    if (!process::ProcessMonitor::IsProcessAlive(updated_process_info.pid)) {
        ...
        // 检查 ZLM 连接是否正常
        if (!zlm_client_) {
            LOG_ERROR("[StreamStartValidator] ZLM 客户端未初始化，流验证失败: {}/{}", target_app, target_stream);
            status = GatewayStatus::Error;
            return false;
        }
        ...
        try {
            ...
            if (!stream_info.app.empty()) {
                // 流存在但未活跃
                LOG_WARN(
                    "[StreamStartValidator] FFmpeg 进程运行中，但 ZLM 中流未活跃: {}/{} (等待时间: {}ms, alive={}, bytesSpeed={}, totalBytes={})，流验证失败",
                    target_app, target_stream, total_wait_time, 
                    stream_info.alive, stream_info.bytes_speed, stream_info.total_bytes);
            } else {
                // 流不存在
                LOG_WARN(
                    "[StreamStartValidator] FFmpeg 进程运行中，但 ZLM 中 {}ms 后仍没有流: {}/{}，流验证失败",
...
bool StreamStartValidator::WaitForStreamReadySimple(
    ...
    if (!process::ProcessMonitor::IsProcessAlive(pid)) {
        ...
        // 获取详细的流信息用于诊断
        LogError(target_app, target_stream, read_error_log_callback); // <--- Add this
        auto stream_info = zlm_client_->GetStreamInfo(target_app, target_stream);
        if (!stream_info.app.empty()) {
            // 流存在但未活跃
            LOG_WARN(
...
bool StreamStartValidator::CheckZLMStream(const std::string& target_app,
                                          const std::string& target_stream) const {
    return ::utils::ZLMStreamChecker::IsStreamAlive(zlm_client_, target_app, target_stream);
}

bool StreamStartValidator::CheckZLMStreamActive(const std::string& target_app,
                                                const std::string& target_stream) const {
    if (!zlm_client_) {
        return false;
    }
...
bool StreamStartValidator::ValidateNative(
    ...
    bool require_active) {
    if (!zlm_client_) {
        LOG_ERROR("[StreamStartValidator] ZLMClient is null when ValidateNative is called");
        status = GatewayStatus::Error;
        return false;
    }
    ...
    if (!start_native_callback(target_app, target_stream, source_url)) {
        LOG_ERROR("[StreamStartValidator] 调用 ZLM API 添加流失败: {}/{}", target_app, target_stream);
        status = GatewayStatus::Error;
        return false;
    }
    ...
    for (int attempt = 0;
        ...
        if (require_active) {
            if (CheckZLMStreamActive(target_app, target_stream)) {
                status = GatewayStatus::Running;
                LOG_INFO("[StreamStartValidator] 原生流已成功建立并活跃: {}/{} (等待时间: {}ms)",
                         target_app, target_stream, total_wait_time);
                return true;
            }
        } else {
            ...
            if (CheckZLMStream(target_app, target_stream)) {
                status = GatewayStatus::Running;
                LOG_INFO("[StreamStartValidator] 原生流已成功建立: {}/{} (等待时间: {}ms)",
                         target_app, target_stream, total_wait_time);
                return true;
            }
        }
    ...
    // 超时后仍没有流或流未活跃
    auto final_stream_info = zlm_client_->GetStreamInfo(target_app, target_stream);
    if (require_active) {
        if (streaming::ZLMClient::IsStreamActive(final_stream_info)) {
            // 最后一次检查成功
            status = GatewayStatus::Running;
            LOG_INFO("[StreamStartValidator] 原生流已成功建立并活跃（超时后）: {}/{} (等待时间: {}ms)",
                     target_app, target_stream, total_wait_time);
            return true;
        } else if (!final_stream_info.app.empty()) {
            // 流记录存在但未活跃，可能是源流不存在或需要更多时间
            // 如果 require_active=true，说明需要流活跃，但流未活跃，应该返回 false
            // 但如果流记录存在，说明 AddStreamProxy 成功了，只是源流可能不存在或需要更多时间
            // 设置为 Starting 状态，让后续的状态检查来更新
            status = GatewayStatus::Starting;
            LOG_WARN("[StreamStartValidator] 原生流已创建但未活跃（{}ms内无数据传输），设置为 Starting 状态: {}/{} (alive={}, bytesSpeed={}, totalBytes={})",
                     total_wait_time, target_app, target_stream,
                     final_stream_info.alive, final_stream_info.bytes_speed, final_stream_info.total_bytes);
            // 注意：即使流未活跃，也返回 true，因为 AddStreamProxy/AddRTMPStream 已成功
            // 后续的状态同步会检查流是否真正活跃，如果长时间不活跃会更新为 Error
            return true;
        } else {
            // 流记录不存在，AddStreamProxy/AddRTMPStream 可能失败了
            LOG_ERROR("[StreamStartValidator] 原生流创建失败，流记录不存在: {}/{} (等待时间: {}ms)",
                      target_app, target_stream, total_wait_time);
            status = GatewayStatus::Error;
            return false;
        }
        ...
        if (CheckZLMStream(target_app, target_stream)) {
            status = GatewayStatus::Running;
            LOG_INFO("[StreamStartValidator] 原生流已成功建立（超时后）: {}/{} (等待时间: {}ms)",
                     target_app, target_stream, total_wait_time);
            return true;
        } else {
            LOG_ERROR("[StreamStartValidator] 原生流创建失败，流记录不存在: {}/{} (等待时间: {}ms)",
