...
StreamInfoResult
StreamInfoResult::FromStreamInfo(const process::StreamInfo &stream_info) {
  ...
  // 如果各个 codec 的 bitrate 为 0，但 format 的 bitrate 不为 0，使用 format 的
  // bitrate
  ...
  if (result.total_bitrate == 0 && result.video_width > 0 &&
      result.video_height > 0 && result.video_fps > 0) {
    // 基于分辨率和帧率估算码率（粗略估算）
    // 公式: bitrate ≈ width * height * fps * 0.1 (粗略估算，单位: bps)
    // 这个估算值用于码率分配，确保不会因为检测失败而使用过低的码率
    int estimated_bitrate = static_cast<int>(
        result.video_width * result.video_height * result.video_fps * 0.1);
    if (estimated_bitrate > 0) {
      result.total_bitrate = estimated_bitrate;
      LOG_INFO("[StreamInfoDetector] 码率检测为0，基于分辨率 {}x{}@{}fps "
               "估算码率: {} bps ({} kbps)",
               result.video_width, result.video_height, result.video_fps,
               estimated_bitrate, estimated_bitrate / 1000);
    }
  }
...
StreamInfoResult StreamInfoDetector::Detect(const std::string &source_url,
  ...
  process::StreamInfo stream_info =
      detector_->DetectStreamInfo(source_url, timeout_seconds);
...
StreamInfoResult StreamInfoDetector::Detect(const std::string &source_url,
  ...
  process::StreamInfo stream_info = detector_->DetectStreamInfo(
      source_url, username, password, timeout_seconds);
  return StreamInfoResult::FromStreamInfo(stream_info);
}

bool StreamInfoDetector::IsVideoCodecCompatible(
  ...
  return lower_codec == "h264" || lower_codec == "libx264" ||
         lower_codec == "h265" || lower_codec == "hevc" ||
         lower_codec == "libx265";
}

bool StreamInfoDetector::IsAudioCodecCompatible(
...
// 辅助函数：不区分大小写的列表查找
static bool IsInList(const std::vector<std::string> &list,
  ...
  for (const auto &item : list) {
    ...
    // 特殊处理 profile：如果列表中是 "Baseline"，那么 "Constrained Baseline"
    // 也应该算作匹配 (包含了 Baseline 关键词)
    // 或者反过来，严格匹配。为了安全起见，这里采用严格匹配 +
    // 宽容模式（用户配置决定） 实际上 FFprobe 返回的 Profile 字符串需要精确匹配
  }
  return false;
}

bool StreamInfoDetector::IsWebRTCCompatible(
  ...
  std::transform(lower_codec.begin(), lower_codec.end(), lower_codec.begin(),
                 ::tolower);
  if (lower_codec.find("h264") != std::string::npos ||
      lower_codec.find("264") != std::string::npos) {
    if (!info.video_profile.empty()) {
      ...
      return false;
      // 修正：如果 profile 缺失，可能是 FFprobe 版本问题或者流问题。
      // 暂且认为不兼容以保证安全
    }
  }
