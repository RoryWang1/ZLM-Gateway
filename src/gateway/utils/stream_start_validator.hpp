...
public:
   ...
   * @param start_native_callback   启动原生流的回调（调用 ZLM
   * API，返回是否成功）
   * @param require_active          是否要求流必须活跃（有数据传输），默认 true
   * @return 是否成功
   */
  bool
  ValidateNative(GatewayStatus &status, const std::string &target_app,
                 const std::string &target_stream,
...
private:
  ...
  void LogError(const std::string &target_app, const std::string &target_stream,
