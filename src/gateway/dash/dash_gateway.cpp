...
namespace {
static gateway::utils::AutoRegister reg("dash", [](const gateway::utils::GatewayContext& ctx) -> std::shared_ptr<gateway::GatewayBase> {
...
DASHGateway::DASHGateway(std::shared_ptr<config::Config> config,
...
Result<void> DASHGateway::Start(const std::string& source_url,
    ...
    std::string resolved_output = info.output_protocol.empty() ? "dash" : info.output_protocol;

    // DASH Gateway 总是使用 FFmpeg 转码（不支持直接代理）
    // 使用 SmartStreamProcessor 处理流启动
    auto result = smart_processor_->ProcessStream(
        source_url, target_app, target_stream, resolved_output, "dash", "dash_gateway",
        // 直接代理回调（DASH 不支持直接代理）
        ...
        [this](const std::string& app, const std::string& stream, const std::string& schema) {
            return zlm_client_ ? zlm_client_->GetStreamInfo(app, stream, schema.empty() ? "dash" : schema) : streaming::StreamInfo();
        },
        // 启动 FFmpeg 回调
        [this, &info, stream_id, resolved_output](const gateway::utils::StreamInfoResult& stream_info_result) {
            ...
            std::string log_file = "/tmp/ffmpeg_dash_" + info.target_app + "_" + info.target_stream + ".log";
...
std::string DASHGateway::BuildFFmpegCommand(const StreamInfo& info, 
    ...
    if (info.output_protocol == "webrtc") {
        ...
        LOG_DEBUG("DASH Gateway: 使用 WebRTC 兼容转码参数，码率: {}k", final_bitrate);
        ...
        LOG_DEBUG("DASH Gateway: 使用优化转码参数 (分辨率: {}x{}, {}k, GOP={})", 
...
std::string DASHGateway::BuildOptimizedFFmpegCommand(const StreamInfo& info, 
                                                     const gateway::utils::StreamInfoResult& stream_info_result,
                                                     int bitrate_kbps) {
    ...
    if (!stream_info_result.valid) {
        LOG_DEBUG("[DASHGateway] StreamInfoDetector 检测失败，使用默认参数: {}", info.source_url);
        return "";
    }

    LOG_INFO("[DASH Gateway] Detected stream info - Audio: {} ({}Hz, {}ch, {}kbps), Video: {} ({}x{}@{}fps, {}kbps), Total: {}kbps", 
    ...
    bool can_use_video_copy = gateway::utils::StreamInfoDetector::CanUseVideoCopy(stream_info_result);
    ...
    if (can_use_copy) {
        ...
        if (info.output_protocol == "http-flv" || info.output_protocol == "hls" || info.output_protocol == "rtsp") {
            ...
            if (can_use_video_copy) {
                ...
                if (stream_info_result.video_width > 0 && stream_info_result.video_height > 0) {
                    LOG_INFO("[DASHGateway] 使用智能转码参数（只转码音频），保持源流分辨率 {}x{}: {}", 
                            stream_info_result.video_width, stream_info_result.video_height, info.source_url);
                } else {
                    LOG_DEBUG("[DASHGateway] 使用智能转码参数（只转码音频）: {}", info.source_url);
                }
                // 音频转码参数
            ...
            LOG_DEBUG("[DASHGateway] 使用转码参数（基于检测结果）: {}", info.source_url);
        }
        // 统一推 RTMP/FLV 到 ZLM
        oss << " -f flv"
            << " \"" << ::utils::zlm_url_builder::BuildRTMPUrlWithSecret(
