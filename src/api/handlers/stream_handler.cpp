...
StreamHandler::StreamHandler(
    ...
    : zlm_client_(zlm_client), stream_manager_(stream_manager),
  ...
  local_camera_gateway_ =
      gateways.Get<gateway::LocalCameraGateway>("local_camera");
...
StreamHandler::SelectGateway(const std::string &protocol) {
  auto it = gateways_.find(protocol);
  if (it != gateways_.end()) {
    return it->second;
  }

  LOG_ERROR("SelectGateway: Unsupported protocol or Gateway not enabled: {}",
            protocol);
  return nullptr;
}

// Phase 1.3 Refactored HandleGetStreams - Complete Version
// This file contains the refactored version to be inserted into
// stream_handler.cpp

void StreamHandler::HandleGetStreams(const httplib::Request & /* req */,
  ...
  try {
    ...
    // Phase 1.3: Use ForEachStream - eliminates vector copy, reduces memory
    // 50-60%
    stream_manager_->ForEachStream(
        [&](const streaming::StreamMetadata &metadata) {
          // Early termination
          if (response_array.size() >= MAX_STREAMS) {
            ::utils::Logger::Get()->warn("Stream list truncated at {} items",
                                         MAX_STREAMS);
            return false;
          }
          ...
          if (metadata.gateway_type == "gb28181_gateway" && gb28181_gateway_) {
            if (!metadata.zlm_stream.empty()) {
              ...
              if (last_underscore != std::string::npos &&
                ...
                std::string suffix =
                    metadata.stream.substr(last_underscore + 1);
                if (suffix.length() >= 10 &&
              ...
              if (!has_timestamp) {
                try {
                  std::string rtp_id = gb28181_gateway_->GetRtpStreamId(
                      metadata.app, metadata.stream);
                  if (rtp_id.empty() && metadata.app != "live") {
                    rtp_id = gb28181_gateway_->GetRtpStreamId("live",
                                                              metadata.stream);
                    if (!rtp_id.empty()) {
                      app_name = "live";
                    }
          ...
          try {
            response_array.push_back(
                BuildStreamJson(metadata, app_name, stream_name));
...
void StreamHandler::HandleStartStream(const httplib::Request &req,
                                      httplib::Response &res) {
  try {
    // 1. 快速参数解析和验证 (<5ms)
    json body;
    ...
    if (body.contains("retry_count") &&
        !body["retry_count"].is_number_integer()) {
      api::utils::ResponseHelper::BadRequest(res,
    ...
    if (stream.empty() || source_url.empty()) {
      api::utils::ResponseHelper::BadRequest(
          res, "stream and source_url cannot be empty");
      return;
    }
    ...
    if (protocol == "rtsp") {
      std::string inferred =
          streaming::StreamManager::InferProtocolFromSourceUrl(source_url);
      if (!inferred.empty()) {
        protocol = inferred;
        LOG_INFO("Phase 2.1: Inferred protocol '{}' from source URL: {}",
                 protocol, source_url);
      }
    }
    ...
    if (!gateway) {
      api::utils::ResponseHelper::Error(
          res, -1, "Unsupported protocol or Gateway not enabled: " + protocol,
          400);
      return;
    }
    ...
    if (!stream_start_queue_) {
      ...
      LOG_WARN(
          "StreamStartQueue not available, using synchronous mode for {}/{}",
          app, stream);
      ...
      try {
        ...
        if (!success) {
          LOG_ERROR("Sync stream start failed: {}/{} - {}", app, stream,
                    result.Error().what());
        }
      } catch (const std::exception &e) {
        LOG_ERROR("Sync stream start failed: {}/{} - {}", app, stream,
      ...
      if (success) {
        ...
        stream_manager_->UpdateStreamStatus(app, stream,
                                            streaming::StreamStatus::Error);
        api::utils::ResponseHelper::Error(res, 500, "Failed to start stream",
                                          500);
      }
      return;
    }
    ...
    task.execute_func = [gateway, source_url, app, stream,
                         output_protocol]() -> gateway::Result<void> {
      try {
        return gateway->Start(source_url, app, stream, output_protocol);
      } catch (const std::exception &e) {
        LOG_ERROR("Async stream start exception: {}/{} - {}", app, stream,
                  e.what());
        return gateway::Result<void>::Failure(
            gateway::InternalServerException(e.what()));
      }
    };

    // 7. 设置回调函数 (处理启动结果)
    task.callback = [this, app, stream](bool success,
      ...
      if (success) {
        ...
        LOG_INFO("Phase 2.2: Stream {}/{} started successfully (async)", app,
                 stream);
      } else {
        // 失败：更新状态为Error
        stream_manager_->UpdateStreamStatus(app, stream,
        ...
        LOG_ERROR("Phase 2.2: Stream {}/{} failed to start (async): {}", app,
                  stream, error);
      }
    ...
    if (!enqueued) {
      ...
      api::utils::ResponseHelper::Error(
          res, -1, "Server busy, please try again later", 503);
      return;
    }
    ...
    LOG_INFO("Phase 2.2: API response sent immediately for {}/{}, background "
             "processing started",
             app, stream);
...
void StreamHandler::HandleStopStream(const httplib::Request &req,
                                     httplib::Response &res) {
  try {
    // 支持两种路由方式：
    ...
    if (req.path_params.find("app") != req.path_params.end() &&
      ...
      try {
        ...
        if (!body.contains("app") || !body.contains("stream")) {
          api::utils::ResponseHelper::BadRequest(
              res, "Missing required parameters: app and stream");
          return;
        }
    ...
    auto metadata = stream_manager_->GetStreamMetadata(app, stream);
    std::string actual_stream_for_unregister =
        stream; // 保存实际用于UnregisterStream的流名称

    // 检查流是否存在（app 和 stream 都为空表示不存在）
    // 如果流不在 StreamManager 中，但可能在 ZLM
    // 中（如直接推送的源流），尝试直接从 ZLM 删除
    ...
    if (!stream_in_manager) {
      // 流不在 Stream Manager 中，可能是直接推送到 ZLM
      // 的源流，或者GB28181流已被Gateway清理但仍在ZLM中
      // 对于GB28181流，尝试通过Gateway的Stop方法清理
      // 注意：即使流不在StreamManager中，如果app是"gb28181"，也应该尝试通过Gateway清理
      if ((metadata.gateway_type == "gb28181_gateway" || app == "gb28181") &&
          gb28181_gateway_) {
        ::utils::Logger::Get()->info(
            "StreamHandler: "
            "流不在StreamManager中，但app是gb28181，尝试通过Gateway清理: {}/{}",
            app, stream);
        // 直接调用Stop，Gateway内部会处理时间戳匹配和RTP流ID查找
        bool gateway_stopped = gb28181_gateway_->Stop(app, stream).IsSuccess();

        if (gateway_stopped) {
          api::utils::ResponseHelper::Success(
              res, json::object(),
              "Stream stopped successfully (via Gateway cleanup)");
          return;
        }
      }
      ...
      if (zlm_deleted) {
        api::utils::ResponseHelper::Success(
            res, json::object(),
            "Stream deleted from ZLMediaKit (not in StreamManager)");
        return;
      } else {
        // ZLM 中也没有，但删除操作应该是幂等的，返回成功
        api::utils::ResponseHelper::Success(
            res, json::object(), "Stream not found (may already be deleted)");
        return;
      }
    }
    ...
    if (stream_stopped) {
      ...
      json response = {{"code", 0}, {"msg", "Stream already stopped"}};
    ...
    // 更新流状态为停止中
    stream_manager_->UpdateStreamStatus(app, stream,
                                        streaming::StreamStatus::Stopping);
    ...
    if (metadata.gateway_type == "httpflv_gateway" && httpflv_gateway_) {
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "HTTP-FLV Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}",
            app, stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "DASH Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "HLS Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "QUIC Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "RTSP Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "ONVIF Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "ISAPI Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "Dahua Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
      ...
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn(
            "PSIA Gateway 停止成功，但清理 ZLMediaKit 流记录失败: {}/{}", app,
            stream);
      }
    } else if (metadata.gateway_type == "local_camera_gateway" &&
               local_camera_gateway_) {
      // Local Camera Gateway：先停止 FFmpeg 进程，然后清理 ZLMediaKit
      // 中的流记录
      success = local_camera_gateway_->Stop(app, stream).IsSuccess();
      zlm_cleaned = zlm_client_->DeleteStream(app, stream);
      if (!zlm_cleaned) {
        ::utils::Logger::Get()->warn("Local Camera Gateway 停止成功，但清理 "
                                     "ZLMediaKit 流记录失败: {}/{}",
                                     app, stream);
      }
    } else if (metadata.gateway_type == "gb28181_gateway" && gb28181_gateway_) {
      // GB28181
      // Gateway：使用GB28181特有的停止逻辑（发送BYE请求、关闭RTP服务器）
      ...
      // 2.
      // 发送两个WebSocket通知：一个基础名称，一个带时间戳的名称（如果stream包含时间戳）
      // 这样确保前端无论使用哪个名称都能匹配上
      ...
      if (unregistered) {
        ::utils::Logger::Get()->info(
            "StreamHandler: 使用原始stream名称注销成功: {}/{}", app, stream);
      } else {
        // 如果失败，尝试使用actual_stream_for_unregister（已处理过时间戳匹配的流名称）
        unregistered = stream_manager_->UnregisterStream(
            app, actual_stream_for_unregister);
        if (unregistered) {
          ::utils::Logger::Get()->info(
              "StreamHandler: 使用actual_stream_for_unregister注销成功: {}/{}",
              app, actual_stream_for_unregister);
        } else {
          ...
          if (last_underscore != std::string::npos) {
            ...
            if (is_timestamp) {
              ...
              unregistered =
                  stream_manager_->UnregisterStream(app, base_stream);
              if (unregistered) {
                ::utils::Logger::Get()->info(
                    "StreamHandler: 使用基础名称注销成功: {}/{} -> {}/{}", app,
                    stream, app, base_stream);
              }
            }
          }
        }
      ...
      if (!unregistered && !(metadata.app.empty() && metadata.stream.empty())) {
        ::utils::Logger::Get()->warn("StreamHandler: "
                                     "所有UnregisterStream尝试都失败，但流存在"
                                     "，强制删除: {}/{} (metadata.stream={})",
        ...
        if (unregistered) {
          ::utils::Logger::Get()->info(
              "StreamHandler: 使用metadata.stream强制删除成功: {}/{}", app,
              metadata.stream);
        } else {
          // 如果metadata.stream也失败，说明流可能已经被删除了，或者有其他问题
          // 但无论如何，我们已经尝试了所有可能的方法
          ::utils::Logger::Get()->error(
              "StreamHandler: 即使使用metadata.stream也无法删除流: {}/{} "
              "(metadata.stream={})，可能流已被删除",
              app, stream, metadata.stream);
        }
      } else if (!unregistered) {
        // 如果metadata为空，说明流确实不存在，这是正常的（可能已经被删除了）
        ::utils::Logger::Get()->debug(
            "StreamHandler: 流不存在于StreamManager中，可能已被删除: {}/{}",
            app, stream);
      }
      ...
      try {
        ...
        rtp_stream_id =
            gb28181_gateway_->GetRtpStreamId(app, actual_stream_for_unregister);
        ...
        ::utils::Logger::Get()->warn(
            "StreamHandler: 获取rtp_stream_id时出现异常，使用stream名称: {} - "
            "{}",
            stream, e.what());
        rtp_stream_id = stream;
      }
      ...
      ::utils::Logger::Get()->info(
          "StreamHandler: 删除GB28181流 {}/{}, rtp_stream_id={} (异步清理)",
          app, stream, rtp_stream_id);

      // 异步执行Stop和DeleteStream操作（避免阻塞API响应）
      std::thread cleanup_thread([this, app, stream, rtp_stream_id,
                                  actual_stream_for_unregister]() {
        try {
          // 尝试停止流
          bool stop_success = false;
          if (!rtp_stream_id.empty() && rtp_stream_id != stream) {
            stop_success =
                gb28181_gateway_->Stop(app, rtp_stream_id).IsSuccess();
            if (!stop_success) {
          ...
          ::utils::Logger::Get()->info(
              "StreamHandler: GB28181流异步清理完成: {}/{} (stop_success={})",
              app, stream, stop_success);
        } catch (const std::exception &e) {
          ::utils::Logger::Get()->error(
              "StreamHandler: GB28181流异步清理时出现异常: {}/{} - {}", app,
              stream, e.what());
        }
      });
      ...
      if (local_camera_gateway_ &&
          local_camera_gateway_->IsRunning(app, stream)) {
        success =
            local_camera_gateway_->Stop(app, stream).IsSuccess() || success;
      }
    ...
    if (success || process_stopped) {
      ...
      bool unregistered =
          stream_manager_->UnregisterStream(app, actual_stream_for_unregister);
      if (!unregistered) {
        ...
        if (!unregistered) {
          ::utils::Logger::Get()->warn("StreamHandler: "
                                       "无法从StreamManager注销流，尝试了所有可"
                                       "能的流名称格式: {}/{} (actual: {})",
                                       app, stream,
                                       actual_stream_for_unregister);
        } else {
          ::utils::Logger::Get()->info(
              "StreamHandler: 使用备用流名称注销成功: {}/{}", app, stream);
        }
      } else {
        ::utils::Logger::Get()->info(
            "StreamHandler: 使用actual_stream_for_unregister注销成功: {}/{}",
      ...
      if (!unregistered && metadata.gateway_type == "gb28181_gateway" &&
        ...
        std::string rtp_stream_id =
            gb28181_gateway_->GetRtpStreamId(app, stream);
        if (!rtp_stream_id.empty() && rtp_stream_id != stream) {
          // 尝试去掉时间戳，获取基础名称
          size_t last_underscore = stream.find_last_of('_');
          if (last_underscore != std::string::npos) {
            ...
            if (!unregistered) {
              ...
              if (rtp_last_underscore != std::string::npos) {
                std::string rtp_base =
                    rtp_stream_id.substr(0, rtp_last_underscore);
      ...
      api::utils::ResponseHelper::Success(res, json::object(),
...
void StreamHandler::HandleGetStreamInfo(const httplib::Request &req,
                                        httplib::Response &res) {
  try {
    ...
    if (metadata.pid > 0 && process_manager_) {
      try {
        ...
        ::utils::Logger::Get()->debug(
            "Failed to get process info for {}/{}: {}", metadata.app,
            metadata.stream, e.what());
      }
    }
    ...
    json response = {
        ...
        {"data",
         {{"app", metadata.app},
          ...
          {"output_protocol",
           metadata.output_protocol.empty() ? "" : metadata.output_protocol},
          {"source_url", metadata.source_url},
          {"device_id", metadata.device_id},
          {"device_type", metadata.device_type},
          {"gateway_type", metadata.gateway_type},
          {"processing_type",
           metadata.processing_type.empty() ? "0" : metadata.processing_type},
          {"transcoding_reason", metadata.transcoding_reason.empty()
                                     ? ""
                                     : metadata.transcoding_reason},
          ...
          {"error_code",
           metadata.error_code.empty() ? "" : metadata.error_code},
          {"error_message",
           metadata.error_message.empty() ? "" : metadata.error_message}}}};
...
bool StreamHandler::CreateStreamOnDemand(const std::string &app,
  ...
  try {
    ...
    auto it = gateways_.find(protocol);
    if (it == gateways_.end()) {
      LOG_ERROR("[StreamHandler] 按需拉流: 不支持的协议或 Gateway 未启用: {}",
                protocol);
      return false;
    }
    ...
    if (success) {
      ...
      LOG_ERROR(
          "[StreamHandler] 按需拉流失败: {}/{} (protocol: {}, source: {})", app,
          stream, protocol, source_url);
    }

    return success;
  } catch (const std::exception &e) {
    ::utils::Logger::Get()->error("Create stream on demand failed: {}",
                                  e.what());
    return false;
  }
}

void StreamHandler::HandleGB28181StartStream(const httplib::Request &req,
                                             httplib::Response &res) {
  try {
    if (!gb28181_gateway_) {
      api::utils::ResponseHelper::Error(res, -1, "GB28181 Gateway not enabled",
                                        500);
      return;
    }
    ...
    if (device_id.empty() || channel_id.empty()) {
      api::utils::ResponseHelper::BadRequest(
          res, "Missing required fields: device_id or channel_id");
      return;
    }
    ...
    auto result = gb28181_gateway_->Start(source_url, target_app, target_stream,
                                          output_protocol);
    bool success = result.IsSuccess();

    if (success) {
      json data = {{"app", target_app},
                   {"stream", target_stream},
                   {"source_url", source_url},
                   {"output_protocol", output_protocol}};
      api::utils::ResponseHelper::Success(
          res, data, "GB28181 stream started successfully");
    } else {
      api::utils::ResponseHelper::Error(
          res, -1,
          "Failed to start GB28181 stream. Device might be offline or stream "
          "already exists.",
          500);
    }
  } catch (const json::exception &e) {
...
void StreamHandler::HandleGB28181StopStream(const httplib::Request &req,
                                            httplib::Response &res) {
  try {
    if (!gb28181_gateway_) {
      api::utils::ResponseHelper::Error(res, -1, "GB28181 Gateway not enabled",
    ...
    if (!req.body.empty()) {
      try {
        ...
        ::utils::Logger::Get()->error("Parse request JSON failed: {}",
                                      e.what());
        api::utils::ResponseHelper::BadRequest(res, "Invalid JSON format: " +
                                                        std::string(e.what()));
        return;
    ...
    if (device_id.empty()) {
      api::utils::ResponseHelper::BadRequest(
          res, "Missing required field: device_id. Please provide device_id in "
               "request body, query parameter, or URL path.");
      return;
    }
    ...
    if (channel_id.empty()) {
      ...
      if (stopped_count > 0) {
        json data = {{"stopped_count", stopped_count},
                     {"device_id", device_id}};
        api::utils::ResponseHelper::Success(
            res, data, "GB28181 streams stopped successfully");
      } else {
        // 即使没有停止任何流，也返回成功（幂等性）
        json data = {{"stopped_count", 0}, {"device_id", device_id}};
        api::utils::ResponseHelper::Success(
            res, data, "No active streams found for this device");
      }
      return;
    }
    ...
    int stopped_count =
        gb28181_gateway_->StopStreamByDevice(device_id, channel_id);

    if (stopped_count > 0) {
      json data = {{"app", target_app}, {"stream", target_stream}};
      api::utils::ResponseHelper::Success(
          res, data, "GB28181 stream stopped successfully");
    } else {
      api::utils::ResponseHelper::Error(
          res, -1, "Stream not found or failed to stop", 404);
    }
  } catch (const json::exception &e) {
    ::utils::Logger::Get()->error("Parse request JSON failed: {}", e.what());
    api::utils::ResponseHelper::BadRequest(res, "Invalid JSON format: " +
...
bool StreamHandler::ShouldIncludeStream(
    const streaming::StreamMetadata &metadata) const {
  // Only show streams created through Gateway API
  return (!metadata.gateway_type.empty() &&
...
nlohmann::json
StreamHandler::BuildStreamJson(const streaming::StreamMetadata &metadata,
  ...
  stream_json["processing_type"] =
      metadata.processing_type.empty() ? "0" : metadata.processing_type;
  stream_json["transcoding_reason"] =
      metadata.transcoding_reason.empty() ? "" : metadata.transcoding_reason;
  ...
  stream_json["error_code"] =
      metadata.error_code.empty() ? "" : metadata.error_code;
  stream_json["error_message"] =
      metadata.error_message.empty() ? "" : metadata.error_message;
