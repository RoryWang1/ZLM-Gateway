...
WebSocketServer::WebSocketServer(int port) : port_(port) {
  ...
  ws_server_.set_open_handler(
      bind(&WebSocketServer::on_open, this, std::placeholders::_1));
  ws_server_.set_close_handler(
      bind(&WebSocketServer::on_close, this, std::placeholders::_1));
  ws_server_.set_message_handler(bind(&WebSocketServer::on_message, this,
                                      std::placeholders::_1,
                                      std::placeholders::_2));
...
bool WebSocketServer::Start() {
  try {
      ...
      try {
        ...
        if (error_msg.find("Address already in use") != std::string::npos ||
          ...
          ::utils::Logger::Get()->warn(
              "WebSocket端口 {} 已被占用，可能已有实例在运行", port_);
        } else {
    ...
    if (error_msg.find("Address already in use") != std::string::npos ||
        ...
        error_msg.find("asio.system:48") != std::string::npos) {
      ::utils::Logger::Get()->warn(
          "WebSocket端口 {} 已被占用，可能已有实例在运行，跳过启动", port_);
    } else {
...
void WebSocketServer::BroadcastMessage(const std::string &message) {
  ...
  if (!connections_.empty()) {
    LOG_INFO("Broadcasting message to {} clients: {}", connections_.size(),
             message);
  }
