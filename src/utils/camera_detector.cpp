...
std::vector<CameraDevice> CameraDetector::DiscoverCamerasMacOS() {
  ...
  std::string command =
      ffmpeg_path_ + " -f avfoundation -list_devices true -i \"\" 2>&1";
  ...
  while (std::getline(iss, line)) {
    ...
    if (in_video_section) {
      ...
      if (last_bracket_start != std::string::npos &&
          last_bracket_end != std::string::npos &&
          last_bracket_end > last_bracket_start) {
        try {
          std::string index_str =
              line.substr(last_bracket_start + 1,
          ...
          if (is_numeric && !index_str.empty()) {
            ...
            if (!device_name.empty()) {
              ...
              LOG_INFO(
                  "[CameraDetector] macOS: 发现摄像头 [{}] {} (device_id: {})",
                  device_index, device_name, device.device_id);
            } else {
          ...
          LOG_WARN("[CameraDetector] macOS: 解析设备索引失败: {}, 行: {}",
                   e.what(), line);
        }
      }
    }
...
std::vector<CameraDevice> CameraDetector::DiscoverCamerasLinux() {
  ...
  // 方法1: 扫描 /sys/class/video4linux 获取更详细信息
  namespace fs = std::filesystem;
  std::string sys_class_v4l = "/sys/class/video4linux";

  if (fs::exists(sys_class_v4l)) {
    for (const auto &entry : fs::directory_iterator(sys_class_v4l)) {
      std::string device_dir_name = entry.path().filename().string();
      // 只要 videoX 设备
      if (device_dir_name.find("video") == 0) {
        try {
          std::string index_str = device_dir_name.substr(5);
          int index = std::stoi(index_str);
          std::string device_path = "/dev/" + device_dir_name;
          ...
          if (device_name.empty()) {
            device_name = "Video Device " + index_str;
          }
          ...
          if (fs::exists(device_path) && fs::is_character_file(device_path)) {
            ...
            if (fd != -1) {
              ...
              if (ioctl(fd, VIDIOC_QUERYCAP, &cap) == 0) {
                if (!(cap.capabilities &
                      V4L2_CAP_VIDEO_CAPTURE)) { // Use capabilities or
                                                 // device_caps
                  // Try device_caps first if available, but for simplicity
                  // check capabilities/device_caps union behavior V4L2 spec
                  // says device_caps should be checked if set. For now, simple
                  // check:
                }
                // If device_caps is present (V4L2_CAP_DEVICE_CAPS), use that.
            ...
            device.device_id =
                GenerateDeviceID(device.name, "linux", device_path);

            devices.push_back(device);
            LOG_DEBUG("[CameraDetector] Linux: 发现摄像头 [{}] {} ({})", index,
                      device_name, device_path);
          }
        } catch (...) {
          LOG_WARN("[CameraDetector] Linux: 解析设备失败: {}", device_dir_name);
        }
      }
    }
  } else {
    // Fallback 到 /dev/video* 扫描
    for (int i = 0; i < 32; ++i) {
...
std::string
CameraDetector::ExecuteFFmpegCommand(const std::string &command) const {
  ...
  if (pid < 0) {
    ...
    // 如果输出为空，记录错误
    if (WIFEXITED(status)) {
      LOG_WARN("[CameraDetector] 命令执行失败，退出码: {}, 无输出",
               WEXITSTATUS(status));
    } else {
...
std::string
CameraDetector::GenerateDeviceID(const std::string &name,
        ...
        try {
          ...
          if (is_numeric && !index_str.empty()) {
            ...
            if (!audio_name.empty()) {
              ...
              LOG_DEBUG("[CameraDetector] macOS: 发现音频设备 [{}] {}",
                        audio_index, audio_name);
            }
          }
        } catch (const std::exception &e) {
          LOG_DEBUG("[CameraDetector] macOS: 解析音频设备索引失败: {}",
                    e.what());
        }
      }
    }
...
#elif __linux__
  // Linux: 读取 /proc/asound/cards 获取音频设备列表
  std::string cards_path = "/proc/asound/cards";
  std::ifstream file(cards_path);
  if (file.is_open()) {
    std::string line;
    while (std::getline(file, line)) {
      // 格式:  0 [PCH            ]: HDA-Intel - HDA Intel PCH
      ...
      if (open_bracket != std::string::npos &&
        ...
        try {
          // index处理: " 0 [PCH..."
          std::string index_part = line.substr(0, open_bracket);
          // trim spaces
          size_t first_digit = index_part.find_first_of("0123456789");
          if (first_digit != std::string::npos) {
            int index = std::stoi(index_part.substr(first_digit));
            std::string name = line.substr(close_bracket + 3); // Skip "] : "

            AudioDevice device;
            device.index = index;
            device.name = name;
            audio_devices.push_back(device);
            LOG_DEBUG("[CameraDetector] Linux: 发现音频设备 [{}] {}", index,
                      name);
          }
        } catch (...) {
        }
    ...
    LOG_WARN("[CameraDetector] Linux: 无法读取 /proc/asound/cards");
  }
#endif
...
int CameraDetector::MatchAudioDevice(
  ...
  // 匹配策略：
  // 1. 精确匹配：音频设备名称包含摄像头名称（例如 "1080P USB Camera" ->
  // "1080P USB Camera-Audio"）
  ...
  for (const auto &audio : audio_devices) {
    ...
    std::transform(audio_name_lower.begin(), audio_name_lower.end(),
                   audio_name_lower.begin(), ::tolower);

    if (audio_name_lower.find(camera_name_lower) != std::string::npos) {
      LOG_DEBUG("[CameraDetector] 音频设备匹配（精确）: 摄像头 '{}' -> "
                "音频设备 [{}] '{}'",
  ...
  for (const auto &audio : audio_devices) {
    ...
    if (audio_name_lower == expected_suffix) {
      LOG_DEBUG("[CameraDetector] 音频设备匹配（后缀）: 摄像头 '{}' -> "
                "音频设备 [{}] '{}'",
                camera_name, audio.index, audio.name);
      return audio.index;
    }
  ...
  // 策略3: 关键词匹配（提取摄像头名称的关键词，如 "1080P", "USB",
  // "Camera"）
  ...
  if (!keywords.empty()) {
    for (const auto &audio : audio_devices) {
      ...
      if (all_keywords_match) {
        LOG_DEBUG("[CameraDetector] 音频设备匹配（关键词）: 摄像头 '{}' -> "
                  "音频设备 [{}] '{}'",
                  camera_name, audio.index, audio.name);
        return audio.index;
      }
...
CameraDetector::QueryCameraCapabilities(int camera_index) const {
  ...
  // 但这样会很慢，所以这里只返回常见值
  LOG_DEBUG("[CameraDetector] macOS: "
            "返回常见分辨率和帧率（设备能力查询需要实际测试）");
#elif __linux__
  // Linux: 使用 v4l2 查询设备能力
  std::string device_path = "/dev/video" + std::to_string(camera_index);
  std::string command =
      ffmpeg_path_ + " -f v4l2 -list_formats all -i " + device_path + " 2>&1";
  std::string output = ExecuteFFmpegCommand(command);

  if (!output.empty()) {
    // 解析输出，提取支持的分辨率和帧率
    // 格式示例: [v4l2 @ ...] Raw       :     yuyv422 : YUYV 4:2:2 :
  ...
  LOG_DEBUG("[CameraDetector] Linux: 查询到 {} 个支持的分辨率",
            resolutions.size());
#endif
