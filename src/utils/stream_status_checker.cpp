...
bool StreamStatusChecker::CheckIsRunning(
    int pid, gateway::GatewayStatus current_status,
    std::shared_ptr<streaming::ZLMClient> zlm_client, const std::string &app,
    const std::string &stream, gateway::GatewayStatus &status_out,
    int &pid_out) {
  pid_out = pid;
  ...
  if (pid > 0) {
    ...
    if (zlm_alive) {
      ...
      // 进程运行但 ZLM 中还没有流 (或者 API 返回 False Negative)
      // 如果当前状态已经是 Running，且 API 汇报不活跃，不要立即杀掉流
      // 保持 Running 状态，等待 Hook 更新或后续检查
      if (current_status == gateway::GatewayStatus::Running) {
        status_out = gateway::GatewayStatus::Running;
        return true;
      }
      // 如果还在 Starting，保持 Starting
      return (current_status == gateway::GatewayStatus::Starting);
    }
  } else {
    // 没有进程，只检查 ZLM
    bool zlm_alive = IsStreamAliveInZLM(zlm_client, app, stream);
...
StreamStatusChecker::GetStatus(int pid, gateway::GatewayStatus current_status,
                               std::shared_ptr<streaming::ZLMClient> zlm_client,
                               const std::string &app,
                               const std::string &stream, int &pid_out) {
  pid_out = pid;
...
bool StreamStatusChecker::IsStreamAliveInZLM(
  ...
  try {
    ...
    // 使用 IsStreamActive 方法，它不仅检查 alive 字段，还检查 bytes_speed 和
    // total_bytes 这对于直接代理模式（没有进程）的流很重要
