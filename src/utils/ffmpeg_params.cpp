...
void FFmpegParams::AddWebRTCEncodingParams(std::ostringstream &oss,
  ...
  oss << " -r 30" // 提升到 30fps，与 HTTP-FLV/HLS 一致，充分利用分配的码率
                  ...
                  // 关键优化：
                  // - bframes=1:
                  // 允许1个B帧，提升编码效率（从0改为1，轻微增加延迟但显著提升流畅度）
                  // - ref=2:
                  // 2个参考帧，提升编码质量（从1改为2，轻微增加延迟但提升画质和流畅度）
                  // - rc-lookahead=10:
                  // 少量前瞻，提升编码效率（从0改为10，允许编码器优化帧间关系）
                  // - no-mbtree=0:
                  // 启用宏块树，提升编码效率（从禁用改为启用，提升编码质量）
                  // - scenecut=40:
                  // 启用场景切换检测，提升编码质量（从0改为40，允许在场景切换时插入关键帧）
  ...
  // 注意：不能同时使用 -r 和 -fpsmax，FFmpeg 会报错 "Only one of -fpsmax and -r
  // can be set for a stream" 使用固定帧率 -r 30 而不是
  // -fpsmax，确保稳定的帧率输出
}
