...
inline std::string GetFFmpegPath() {
    ...
    if (env_root) {
        ...
        } else if (fs::exists(current_path.parent_path() / "third_party" / "ffmpeg")) {
            // 如果在 bin/ 目录下
    ...
    // 构建路径
    fs::path ffmpeg_path = fs::path(project_root) / "third_party" / "ffmpeg" / platform / "ffmpeg";
...
inline std::string GetFFprobePath() {
    ...
    fs::path ffprobe_path = fs::path(project_root) / "third_party" / "ffmpeg" / platform / "ffprobe";
