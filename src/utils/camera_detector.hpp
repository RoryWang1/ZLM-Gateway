...
public:
     ...
     */
    int MatchAudioDevice(const std::string& camera_name, const std::vector<AudioDevice>& audio_devices) const;
     ...
     */
    std::pair<std::vector<std::string>, std::vector<int>> QueryCameraCapabilities(int camera_index) const;
...
private:
     ...
     * @return 命令输出
     */
    std::string ExecuteFFmpegCommand(const std::string& command);
     ...
     * @return 设备ID
     */
    std::string GenerateDeviceID(const std::string& name, const std::string& platform, const std::string& device_path = "") const;
