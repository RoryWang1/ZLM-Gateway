...
static int DetectSystemBandwidth() {
  ...
  // 方法1: 查找活跃的网络接口（排除lo0）
  FILE *pipe = popen(
      "route get default 2>/dev/null | grep interface | awk '{print $2}'", "r");
  if (pipe) {
    char buffer[128];
    if (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
      ...
      if (!interface_name.empty() && interface_name != "lo0") {
        ...
        if (interface_name == "en0" || interface_name == "en1") {
          ...
          std::string cmd = "networksetup -getairportnetwork " +
                            interface_name + " 2>/dev/null";
  ...
  pipe = popen("networksetup -listallhardwareports 2>/dev/null | grep -A 1 "
               "'Hardware Port' | grep -E 'Ethernet|Wi-Fi' | head -1",
               "r");
  if (pipe) {
    char buffer[128];
    if (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
  ...
  // 方法3: 查找第一个非lo0的活跃接口
  pipe = popen(
      "ifconfig | grep -E '^[a-z]' | grep -v '^lo0:' | head -1 | cut -d: -f1",
      "r");
  if (pipe) {
    char buffer[128];
    if (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
  ...
  // 查找第一个活跃的网络接口
  FILE *pipe = popen("ip -o link show | grep -v 'lo:' | head -1 | cut -d: -f2 "
                     "| awk '{print $1}'",
                     "r");
  if (!pipe) {
    return 0;
  ...
  if (!interface_name.empty()) {
    ...
    std::string cmd =
        ...
        " 2>/dev/null | grep -i 'Speed:' | awk '{print $2}' | sed 's/Mb\\/s//'";
...
int main(int argc, char *argv[]) {
  ...
  // 初始化日志系统
  utils::Logger::Initialize(config->gateway.log_level,
  ...
  auto zlm_client = std::make_shared<streaming::ZLMClient>(
      config->zlmediakit.api_url, config->zlmediakit.secret);
  ...
  if (!g_websocket_server->Start()) {
    ...
    ::utils::Logger::Get()->warn(
        "WebSocket 服务器启动失败，端口: {} (可能已被占用，不影响主程序运行)",
        ws_port);
  }

  // 创建 Process Manager（统一的 FFmpeg 进程管理）
  auto process_manager = std::make_shared<process::ProcessManager>(
      config->process.max_restarts, config->process.monitor_interval,
      config->process.max_processes);

  // 创建 Stream Manager (传入 WebSocket Server)
  auto stream_manager = std::make_shared<streaming::StreamManager>(
      zlm_client, g_websocket_server);
  ...
  auto bitrate_allocator =
      std::make_shared<utils::BitrateAllocator>(bitrate_config, zlm_client);
  LOG_INFO("智能码率分配器已创建，总带宽: {} Mbps",
           bitrate_config.total_bandwidth_mbps);

  // 使用工厂类创建所有Gateway实例
  auto gateways = gateway::utils::GatewayFactory::CreateAllGateways(
      config, zlm_client, process_manager, stream_manager, bitrate_allocator,
      g_websocket_server);
