...
StreamInfo FFprobeDetector::DetectStreamInfo(const std::string &source_url,
  ...
  if (json_output.empty()) {
    ...
    ::utils::Logger::Get()->debug("FFprobe 执行失败或超时: {} (将使用默认参数)",
                                  source_url);
    return StreamInfo();
  }
...
FFprobeDetector::GenerateOptimizedParams(const StreamInfo &stream_info,
...
bool FFprobeDetector::CanUseCopy(const StreamInfo &stream_info,
                                 const std::string &target_protocol) {
  // 检查所有编码是否兼容目标协议
  for (const auto &codec : stream_info.codecs) {
...
std::string
FFprobeDetector::ExecuteFFprobe(const std::vector<std::string> &args,
                                int timeout_seconds) {
  // 创建管道
  int pipefd[2];
  if (pipe(pipefd) == -1) {
  ...
  if (pid < 0) {
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
      ...
      ::utils::Logger::Get()->warn("ffprobe 执行失败，退出码: {} "
                                   "(源可能不存在或无法访问，将使用默认参数)",
                                   exit_code);
      return "";
    }
      ...
      for (const auto &stream : j["streams"]) {
        ...
        if (stream.contains("sample_rate")) {
          std::string sample_rate_str =
              stream["sample_rate"].get<std::string>();
...
bool FFprobeDetector::IsCodecCompatible(const std::string &codec_name,
                                        const std::string &target_protocol) {
