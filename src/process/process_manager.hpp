 ...
 * @brief 进程监控回调函数类型
 */
using ProcessMonitorCallback =
    std::function<void(const ProcessInfo &info, ProcessErrorType error_type)>;
...
public:
   ...
   */
  ProcessManager(int max_restarts = 3, int monitor_interval = 5,
                 int max_processes = 0);
   ...
   * @param log_file 日志文件路径（空表示不重定向）
   * @param max_restarts 最大重启次数（0 表示使用默认值）
   * @param cpu_limit CPU 限制（百分比，0 表示无限制）
   * @param memory_limit 内存限制（字节，0 表示无限制）
   * @return 是否成功
   */
  bool StartProcess(const std::string &process_id, const std::string &name,
                    const std::string &command, const std::string &source_url,
                    const std::string &target_app,
                    const std::string &target_stream,
                    const std::string &log_file = "", int max_restarts = 0,
                    int cpu_limit = 0, int64_t memory_limit = 0);
