...
ProcessManager::ProcessManager(int max_restarts, int monitor_interval,
                               int max_processes)
  ...
  LOG_INFO("Process Manager 初始化完成 (最大重启次数: {}, 监控间隔: {}s, "
           "最大进程数: {})",
           max_restarts, monitor_interval,
           max_processes > 0 ? std::to_string(max_processes) : "无限制");
}
...
bool ProcessManager::StartProcess(const std::string &process_id,
                                  ...
                                  const std::string &log_file, int max_restarts,
                                  int cpu_limit, int64_t memory_limit) {
  std::lock_guard<std::mutex> lock(mutex_);

  // 检查最大进程数限制
  if (max_processes_ > 0 &&
      static_cast<int>(processes_.size()) >= max_processes_) {
  ...
  try {
    if (!info.log_file.empty()) {
      std::filesystem::path log_path(info.log_file);
      std::filesystem::path parent = log_path.parent_path();
      if (!parent.empty() && !std::filesystem::exists(parent)) {
        std::filesystem::create_directories(parent);
        LOG_DEBUG("创建日志目录: {}", parent.string());
      }
    }
  } catch (const std::exception &e) {
    LOG_ERROR("创建日志目录失败: {}", e.what());
    // 不中断流程，尝试继续（可能会fallback到/dev/null）
  }
...
ProcessInfo
ProcessManager::GetProcessInfo(const std::string &process_id) const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool ProcessManager::StartFFmpegProcess(ProcessInfo &info) {
  ...
  if (pid < 0) {
    ...
    // 重定向输出到日志文件
    int log_fd =
        open(info.log_file.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (log_fd >= 0) {
...
bool ProcessManager::StopFFmpegProcess(ProcessInfo &info) {
  ...
  if (kill(info.pid, SIGKILL) == 0) {
    ...
    LOG_WARN("SIGKILL 后进程仍存在 (PID: {})，可能是 zombie 进程，认为停止成功",
             info.pid);
    info.pid = 0;
    info.status = ProcessStatus::Stopped;
    return true;
  ...
  // 第四步：更新进程信息（加锁更新）
  {
    ...
    if (needs_restart) {
      ...
      if (should_restart) {
        ...
        if (restart_count >= max_restarts) {
          LOG_ERROR("进程超过最大重启次数: {} (已重启 {} 次)", process_id,
                    restart_count);
        } else {
...
ProcessErrorType ProcessManager::AnalyzeError(const ProcessInfo &info) {
  ...
  std::transform(lower_log.begin(), lower_log.end(), lower_log.begin(),
                 ::tolower);
...
bool ProcessManager::TryRestartProcess(const std::string &process_id) {
  ...
  if (info.restart_count >= info.max_restarts) {
    LOG_ERROR("进程超过最大重启次数: {} (已重启 {} 次，最大 {} 次)", process_id,
  ...
  auto time_since_last_restart =
      std::chrono::duration_cast<std::chrono::seconds>(now -
                                                       info.last_restart_time)
          .count();
  ...
  LOG_INFO("尝试重启进程: {} (第 {} 次重启，错误类型: {})", process_id,
           info.restart_count + 1, static_cast<int>(info.last_error));
...
void ProcessManager::ApplyResourceLimits(pid_t pid, int cpu_limit,
  ...
  if (cpu_limit > 0 && pid > 0) {
    ...
    if (setpriority(PRIO_PROCESS, pid, nice_value) != 0) {
      LOG_WARN("设置进程 {} 优先级失败 (nice={}): {}", pid, nice_value,
               strerror(errno));
    } else {
      LOG_DEBUG("设置进程 {} CPU 限制: {}% (nice={})", pid, cpu_limit,
                nice_value);
    }
  }
...
void ProcessManager::UpdateProcessTier(const std::string &process_id) {
  ...
  case ProcessStatus::Running: {
    ...
    auto now = std::chrono::system_clock::now();
    auto runtime =
        std::chrono::duration_cast<std::chrono::seconds>(now - info.start_time)
            .count();
  ...
  if (new_tier != old_tier) {
    ...
    LOG_DEBUG("Phase 2.3: Process {} tier changed: {} -> {} (status: {})",
              process_id, static_cast<int>(old_tier),
              static_cast<int>(new_tier), static_cast<int>(info.status));
  }
}
