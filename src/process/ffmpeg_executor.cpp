...
pid_t FFmpegExecutor::StartProcess(
    const std::string &command, std::shared_ptr<ProcessManager> process_manager,
    const std::string &process_id, const std::string &process_name,
    const std::string &source_url, const std::string &target_app,
    const std::string &target_stream, const std::string &log_file) {
  if (process_manager && !process_id.empty()) {
    ...
    if (process_manager->StartProcess(process_id, process_name, command,
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(time::PROCESS_STABLE_WAIT_MS));
      auto process_info = process_manager->GetProcessInfo(process_id);
      if (process_info.pid > 0) {
        LOG_DEBUG("[FFmpegExecutor] 通过 ProcessManager 启动进程成功 (PID: {})",
                  process_info.pid);
...
pid_t FFmpegExecutor::StartProcessSimple(const std::string &command,
...
bool FFmpegExecutor::StopProcess(
    pid_t pid, std::shared_ptr<ProcessManager> process_manager,
  ...
  if (process_manager && !process_id.empty()) {
    ...
    if (on_status_update) {
      on_status_update(success ? gateway::GatewayStatus::Stopped
...
bool FFmpegExecutor::StopProcessSimple(
    pid_t pid, std::function<void(gateway::GatewayStatus)> on_status_update) {
  if (pid <= 0) {
    return true;
  }
  ...
  if (kill(pid, SIGTERM) != 0) {
    LOG_WARN("[FFmpegExecutor] 发送 SIGTERM 失败 (PID: {}): {}", pid,
             strerror(errno));
    // 如果进程不存在，认为停止成功
    if (errno == ESRCH) {
      if (on_status_update) {
