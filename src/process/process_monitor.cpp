...
std::chrono::system_clock::time_point ProcessMonitor::last_update_time_ = std::chrono::system_clock::now();
...
ProcessResources ProcessMonitor::GetProcessResources(pid_t pid) {
    ...
    if (cached_pid_ == pid && last_system_cpu_time_ > 0 && last_process_cpu_time_ > 0) {
        // 计算时间差
        ...
        if (time_diff > 0) {
            ...
            if (system_diff > 0) {
                ...
                resources.cpu_usage = (static_cast<double>(process_diff) / system_diff) * 100.0;
...
ProcessMonitor::SystemResources ProcessMonitor::GetSystemResources() {
    ...
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vm_info, &count) == KERN_SUCCESS) {
        // Page size is usually 4KB or 16KB
        vm_size_t page_size;
        host_page_size(mach_host_self(), &page_size);
        
        // Used memory = active + wired (simplified)
        // Note: This is a rough estimate. macOS memory management is complex (compressed, etc.)
        resources.memory_usage = (int64_t)(vm_info.active_count + vm_info.wire_count) * page_size;
    }
