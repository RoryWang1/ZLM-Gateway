...
public:
  ...
  GenerateOptimizedParams(const StreamInfo &stream_info,
   ...
   * @return 是否可以使用 copy
   */
  bool CanUseCopy(const StreamInfo &stream_info,
                  const std::string &target_protocol);
...
private:
   ...
   * @return 输出内容
   */
  std::string ExecuteFFprobe(const std::vector<std::string> &args,
                             int timeout_seconds);
   ...
   * @return 是否兼容
   */
  bool IsCodecCompatible(const std::string &codec_name,
                         const std::string &target_protocol);
