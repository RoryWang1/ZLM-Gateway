...
bool ZLMClient::AddRTMPStream(const std::string &app, const std::string &stream,
  ...
  if (success) {
    ...
    if (msg.find("already exists") != std::string::npos) {
      LOG_WARN("添加 RTMP 推流: 流已存在 (视为成功): {}/{} -> {}", app, stream,
               url);
      return true;
    }
...
bool ZLMClient::AddRTSPStream(const std::string &app, const std::string &stream,
                              const std::string &url) {
  json params = {
      {"secret", secret_},
      {"vhost", "__defaultVhost__"},
      ...
      {"schema", ""},
      {"retry_count", 3},
      {"timeout_sec", 30} // RTSP 连接超时时间（秒），30秒适合 RTSP
                          // 连接建立可能需要更长时间的场景
  };
  ...
  if (success) {
    ...
    // 如果流已存在，视为成功（幂等性）
    if (msg.find("already exists") != std::string::npos) {
      LOG_WARN("添加 RTSP 推流: 流已存在 (视为成功): {}/{} -> {}", app, stream,
               url);
      return true;
    }
...
bool ZLMClient::AddStreamProxy(const std::string &app,
  ...
  json params = {{"secret", secret_}, {"vhost", "__defaultVhost__"},
                 {"app", app},        {"stream", stream},
                 {"url", url},        {"schema", ""},
                 {"retry_count", 3},  {"timeout_sec", timeout_sec}};
  ...
  if (success) {
    ...
    // 如果流已存在，视为成功（幂等性）
    if (msg.find("already exists") != std::string::npos) {
      LOG_WARN("添加流代理: 流已存在 (视为成功): {}/{} -> {}", app, stream,
               url);
      return true;
    }
    ...
    if (msg.find("timeout") != std::string::npos ||
        msg.find("wait http response body timeout") != std::string::npos) {
      std::string lower_url_check = url;
      std::transform(lower_url_check.begin(), lower_url_check.end(),
                     lower_url_check.begin(), ::tolower);
      if (lower_url_check.find("http://") == 0 ||
          lower_url_check.find("https://") == 0) {
        // HTTP/HTTPS 协议，即使 API 超时，流可能已经创建
        // 返回 true，让上层继续检查流是否真的存在
        LOG_WARN(
            "添加流代理 API 返回超时，但流可能已创建，继续检查: {}/{} -> {}",
            app, stream, url);
...
bool ZLMClient::DeleteStream(const std::string &app,
                             const std::string &stream) {
  // ZLMediaKit 删除流有两种方式：
  ...
  json params = {{"secret", secret_}, {"key", key}};
  ...
  json close_params = {{"secret", secret_},
                       {"schema", ""}, // 空字符串表示所有协议
                       {"vhost", "__defaultVhost__"},
                       {"app", app},
                       {"stream", stream}};
  ...
  if (close_success) {
    ...
    LOG_ERROR("删除流失败: {}/{} (delStreamProxy 和 close_streams 都失败)", app,
              stream);
    if (close_response.contains("msg")) {
      LOG_ERROR("close_streams 错误信息: {}",
                close_response["msg"].get<std::string>());
    }
    return false;
  }
}
...
StreamInfo ZLMClient::GetStreamInfo(const std::string &app,
                                    const std::string &stream,
...
bool ZLMClient::IsStreamActive(const StreamInfo &stream_info) {
  ...
  return stream_info.alive || stream_info.bytes_speed > 0 ||
         stream_info.total_bytes > 0;
}
...
StreamInfo ZLMClient::ParseStreamInfo(const nlohmann::json &j) {
  ...
  try {
    ...
    std::ofstream log_file(
        "/Users/rory/work/gateway/ZLM-gateway/.cursor/debug.log",
        std::ios::app);
    if (log_file.is_open()) {
      log_file << "{\"timestamp\":"
               << std::chrono::duration_cast<std::chrono::milliseconds>(
                      ...
                      .count()
               << ",\"location\":\"zlm_client.cpp:413\",\"message\":"
                  "\"ParseStreamInfo "
                  "entry\",\"sessionId\":\"debug-session\",\"runId\":\"run1\","
                  "\"hypothesisId\":\"B\"}\n";
      log_file.close();
    }
    // #endregion
    if (j.contains("app") && j["app"].is_string()) {
      ...
      // #region agent log
      log_file.open("/Users/rory/work/gateway/ZLM-gateway/.cursor/debug.log",
                    std::ios::app);
      if (log_file.is_open()) {
        log_file << "{\"timestamp\":"
                 << std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch())
                        .count()
                 << ",\"location\":\"zlm_client.cpp:418\",\"message\":\"Parsed "
                    "app from ZLM\",\"data\":{\"app\":\""
                 << info.app.substr(0, 100)
                 << "\",\"app_length\":" << info.app.length()
      ...
      if (log_file.is_open()) {
        log_file << "{\"timestamp\":"
                 << std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch())
                        .count()
                 << ",\"location\":\"zlm_client.cpp:419\",\"message\":\"Parsed "
                    "stream from ZLM\",\"data\":{\"stream\":\""
                 << info.stream.substr(0, 100)
                 << "\",\"stream_length\":" << info.stream.length()
                 << "},\"sessionId\":\"debug-session\",\"runId\":\"run1\","
                    "\"hypothesisId\":\"B\"}\n";
        log_file.close();
      }
    ...
    if (j.contains("schema") && j["schema"].is_string())
      info.schema = j["schema"].get<std::string>();
    if (j.contains("vhost") && j["vhost"].is_string())
      info.vhost = j["vhost"].get<std::string>();
    if (j.contains("ip") && j["ip"].is_string())
      info.ip = j["ip"].get<std::string>();

    // 数字字段 - 安全转换（处理可能是字符串或数字的情况）
    auto safeGetInt = [](const nlohmann::json &j, const std::string &key,
                         int &target) {
      if (!j.contains(key)) {
        return;
      }
      try {
        ...
        if (val.is_number()) {
          if (val.is_number_unsigned()) {
            ...
            target =
                (uval > static_cast<uint64_t>(std::numeric_limits<int>::max()))
    ...
    // originTypeStr 是字符串，不解析为 int（结构体中定义为 int
    // 但实际是字符串，不使用）
    ...
    // bytesSpeedStr 是字符串，不解析为 int（结构体中定义为 int
    // 但实际是字符串，不使用）
    safeGetInt(j, "totalBytes", info.total_bytes);
    // totalBytesStr 是字符串，不解析为 int（结构体中定义为 int
    // 但实际是字符串，不使用）
    ...
    // 注意：origin_type_str, bytes_speed_str, total_bytes_str 在结构体中定义为
    // int 但 ZLMediaKit 返回的是字符串，这些字段不应该被使用

    // 布尔字段 - alive
    // 注意：ZLMediaKit 的 getMediaList API 可能不返回 alive 字段，或者返回的
    // alive 字段不准确 改进：即使 alive 字段为
    // false，如果有数据传输，也应该认为流是活跃的 这是因为 ZLMediaKit 的 alive
    // 字段可能有延迟，或者在某些情况下不准确
    if (j.contains("alive")) {
      ...
      info.alive = (info.alive_second > 0) || (info.bytes_speed > 0) ||
                   (info.total_bytes > 0);
    }

    // 改进：即使 ZLMediaKit 返回
    // alive=false，如果有数据传输，也应该认为流是活跃的 这是因为 ZLMediaKit 的
    // alive 字段可能有延迟，或者在某些情况下不准确 例如：流代理（stream
    // proxy）可能在创建后立即有数据传输，但 alive 字段可能还是 false
...
RtpServerInfo ZLMClient::OpenRtpServer(const std::string &stream_id,
                                       uint16_t port, int tcp_mode,
                                       bool enable_rtcp,
  ...
  json params = {
      ...
      {"app",
       app.empty() ? "gb28181" : app}, // 使用传入的app参数，默认为gb28181
  ...
  if (success) {
    ...
    // ZLM API返回格式可能是 {"code": 0, "port": 32278} 或 {"code": 0, "data":
    // {"port": 32278}} 需要同时支持两种格式
    ...
    LOG_ERROR("创建RTP服务器失败: stream_id={}, msg={}, response={}", stream_id,
              msg, response_str);
  }
...
bool ZLMClient::CloseRtpServer(const std::string &stream_id,
                               const std::string &app,
  ...
  if (success) {
    LOG_INFO("关闭RTP服务器成功: stream_id={}, app={}, vhost={}", stream_id,
             app, vhost);
  } else {
    std::string msg;
    if (response.contains("msg")) {
...
RtpServerInfo ZLMClient::GetRtpServerInfo(const std::string &stream_id,
                                          const std::string &app,
                                          const std::string &vhost) {
...
RtpInfo ZLMClient::GetRtpInfo(const std::string &stream_id,
                              const std::string &app,
                              const std::string &vhost) {
...
bool ZLMClient::UpdateRtpServerSSRC(const std::string &stream_id,
                                    const std::string &ssrc,
                                    const std::string &app,
                                    const std::string &vhost) {
