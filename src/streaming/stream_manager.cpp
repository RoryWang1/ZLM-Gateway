...
StreamManager::StreamManager(std::shared_ptr<ZLMClient> zlm_client, 
...
std::string StreamManager::GetStreamKey(const std::string& app, const std::string& stream) const {
    return app + "/" + stream;
}
...
static std::string OutputProtocolToSchema(const std::string& output_protocol) {
    if (output_protocol == "http-flv") return "http-flv";
    if (output_protocol == "hls") return "hls";
    if (output_protocol == "rtmp") return "rtmp";
    if (output_protocol == "rtsp") return "rtsp";
    if (output_protocol == "webrtc") return "";  // WebRTC不通过getMediaList查询
    return "";
}
...
bool StreamManager::IsAutoRegistered(const StreamMetadata& existing) {
    return (existing.gateway_type == "native" || existing.gateway_type.empty()) &&
           existing.protocol != "local-camera" &&
           existing.source_url.empty();
}

std::string StreamManager::InferProtocolFromSourceUrl(const std::string& source_url) {
    if (source_url.empty()) return "";
    
    if (source_url.find("rtsp://") == 0) return "rtsp";
    if (source_url.find("rtmp://") == 0) return "rtmp";
    if (source_url.find("http://") == 0 || source_url.find("https://") == 0) {
        if (source_url.find(".m3u8") != std::string::npos) return "hls";
        if (source_url.find(".flv") != std::string::npos) return "http-flv";
        if (source_url.find(".mpd") != std::string::npos) return "dash";
        return "http";
    }
...
void StreamManager::BroadcastStreamUpdate(StreamMetadata& metadata, bool removed, bool force) {
    ...
    // 可以添加更多字段
    LOG_INFO("Broadcasting stream update: app={}, stream={}, status={}", metadata.app, metadata.stream, (int)metadata.status);
    websocket_server_->BroadcastStreamUpdate(json_data);
}

void StreamManager::RecordStatusTransition(const std::string& app, const std::string& stream, StreamStatus from_status, StreamStatus to_status, const std::string& source) {
    ...
    try {
        ...
        // 保持最近转换记录不超过最大值
        if (status_stats_.recent_transitions.size() > StatusStatistics::MAX_RECENT_TRANSITIONS) {
            status_stats_.recent_transitions.erase(status_stats_.recent_transitions.begin());
        }
    } catch (const std::exception& e) {
        LOG_ERROR("[RecordStatusTransition] Exception: {} (app={}, stream={})", e.what(), app, stream);
    }
}
...
std::string StreamManager::StreamStatusToString(StreamStatus status) const {
    switch (status) {
        ...
        case StreamStatus::Stopping: return "Stopping";
...
bool StreamManager::RegisterStream(const StreamMetadata& metadata) {
    ...
    if (streams_.count(key) > 0) {
        ...
        // 如果新注册的流有 gateway_type（说明是通过 Gateway 注册的），优先使用新流的协议信息
        // 这样可以确保本地摄像头等通过 Gateway 注册的流不会被自动注册的流覆盖
    ...
    LOG_INFO("注册流: {} (协议: {}, Gateway: {})", key, metadata.protocol, metadata.gateway_type);
...
bool StreamManager::UnregisterStream(const std::string& app, const std::string& stream) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (it == streams_.end()) {
        ...
        if (app == "gb28181" && stream.find('_') != std::string::npos) {
            ...
            if (gb28181_matcher_) {
                ...
                if (base_name != stream) {
                    ...
                    if (it != streams_.end()) {
                        LOG_DEBUG("UnregisterStream: GB28181 stream matched (O(1)): {} -> {}", stream, base_name);
        ...
        if (it == streams_.end()) {
            ...
            // 尝试记录基础名称
            if (app == "gb28181" && stream.find('_') != std::string::npos && gb28181_matcher_) {
                std::string base_name = gb28181_matcher_->GetKey(stream).base_name;
                if (base_name != stream) {
                    std::string base_key = GetStreamKey(app, base_name);
...
void StreamManager::OnStreamCreateRequested(const StreamMetadata& metadata) {
    ...
    if (it == streams_.end()) {
        ...
        RecordStatusTransition(metadata.app, metadata.stream, StreamStatus::Stopped, StreamStatus::Starting, "gateway_create_requested");
        BroadcastStreamUpdate(new_metadata, false, true);
    } else {
        ...
        UpdateMetadataFields(existing, metadata, false /* preserve_protocols = false */);
        ...
        if (old_status == StreamStatus::Stopped || 
            ...
            LOG_INFO("OnStreamCreateRequested: 状态转移 {} -> Starting (创建请求): {}",
                     static_cast<int>(old_status), key);
            RecordStatusTransition(metadata.app, metadata.stream, old_status, StreamStatus::Starting, "gateway_create_requested");
        } else if (old_status == StreamStatus::Running) {
            // 不允许从 Running 降级到 Starting，保持 Running 状态
            existing.last_update_time = now;
            LOG_WARN("OnStreamCreateRequested: 检测到状态降级尝试 Running -> Starting: {}，"
                     "保持 Running 状态避免降级",
                     key);
        } else {
            // 未知状态，记录告警但允许更新
            LOG_WARN("OnStreamCreateRequested: 检测到未知状态 {}，允许转移到 Starting: {}",
                     static_cast<int>(old_status), key);
            existing.status = StreamStatus::Starting;
            existing.last_update_time = now;
            RecordStatusTransition(metadata.app, metadata.stream, old_status, StreamStatus::Starting, "gateway_create_requested");
...
void StreamManager::OnStreamCreateResult(const std::string& app,
    ...
    // - 如果已经是 Running，不允许降级到 Error（避免误伤已由 Hook 提升为 Running 的流）
    ...
    if (success) {
        ...
        if (old_status == StreamStatus::Starting) {
            existing.status = StreamStatus::Running;
            existing.last_update_time = now;
            LOG_INFO("OnStreamCreateResult: 状态转移 Starting -> Running (创建成功): {}", key);
            RecordStatusTransition(app, stream, old_status, StreamStatus::Running, "gateway_create_result");
        } else if (old_status == StreamStatus::Running) {
            // 已经是 Running，只更新时间戳
            existing.last_update_time = now;
            LOG_DEBUG("OnStreamCreateResult: 创建成功，但流已处于 Running 状态: {}", key);
        } else {
            // 从其他状态转移到 Running（可能是状态机时序问题，记录告警但允许）
            LOG_WARN("OnStreamCreateResult: 检测到非预期的状态转移 {} -> Running (创建成功): {}，"
                     "允许更新但请检查状态机时序",
                     static_cast<int>(old_status), key);
            existing.status = StreamStatus::Running;
            existing.last_update_time = now;
        ...
        if (old_status == StreamStatus::Starting) {
            ...
            LOG_WARN("OnStreamCreateResult: 状态转移 Starting -> Error (创建失败): {} (错误码: {}, 错误消息: {})", 
                     key, error_code.empty() ? "N/A" : error_code, error_message.empty() ? "N/A" : error_message);
            RecordStatusTransition(app, stream, old_status, StreamStatus::Error, "gateway_create_result");
        } else if (old_status == StreamStatus::Running) {
            // 已经是 Running，不允许降级到 Error（避免误伤已由 Hook 提升为 Running 的流）
            existing.last_update_time = now;
            LOG_WARN("OnStreamCreateResult: 创建失败，但流已处于 Running 状态，保持 Running 避免降级: {}", key);
            ...
            LOG_WARN("OnStreamCreateResult: 检测到非预期的状态转移 {} -> Error (创建失败): {}，"
                     "允许更新但请检查状态机时序",
                     static_cast<int>(old_status), key);
            existing.status = StreamStatus::Error;
            existing.last_update_time = now;
...
void StreamManager::OnZLMStreamState(const std::string& app,
    ...
    if (it == streams_.end()) {
        ...
        if (manually_stopped_streams_.count(key) > 0) {
            LOG_DEBUG("OnZLMStreamState: 流在手动停止列表中，忽略状态更新（防止重新注册）: {}", key);
            return;
        }
            ...
            if (last_underscore != std::string::npos && last_underscore < stream.length() - 1) {
                ...
                if (is_timestamp) {
                    ...
                    if (manually_stopped_streams_.count(base_key) > 0) {
                        LOG_DEBUG("OnZLMStreamState: 流（基础名称）在手动停止列表中，忽略状态更新（防止重新注册）: {} -> {}", stream, base_key);
                        return;
                    }
                }
            }
        ...
        LOG_DEBUG("OnZLMStreamState: 流尚未在 StreamManager 中注册，忽略状态更新: {}", key);
        return;
    }
    ...
    // 状态转移规则（根据文档 2.4.2）：
    // - regist && alive -> Running（允许从 Starting/Error/Stopped 转移到 Running）
    // - !regist 或 !alive -> Stopped（允许从 Running/Starting 转移到 Stopped）
    // 但是，如果流刚启动成功（从 Starting 变为 Running 的时间很短），应该更谨慎地处理状态转移
    // 避免在流刚启动时因为 ZLM Hook 的时序问题导致状态被误设为 Stopped
    ...
    // 如果流刚变为 Running（30秒内），且当前状态是 Running，即使 ZLM 上报 regist=false 或 alive=false，
    // 也应该保持 Running 状态，因为可能是 ZLM Hook 的时序问题（流刚启动，ZLM 还没完全注册）
    bool is_recently_running = (old_status == StreamStatus::Running) && (time_since_update < 30);
    
    // 对于Starting状态的流，如果regist=true但alive=false，但流刚创建（60秒内），
    // 应该等待一段时间，因为ZLM的alive字段可能有延迟
    bool is_recently_created = (old_status == StreamStatus::Starting) && (time_since_create < 60);
    ...
    if (regist && effective_alive) {
        ...
        LOG_DEBUG("OnZLMStreamState: 流刚创建（{}秒前），regist=true但alive=false，保持Starting状态等待alive字段更新: {} regist={} alive={} bytes_speed={} total_bytes={}",
        ...
        if (is_recently_running && old_status == StreamStatus::Running) {
            new_status = StreamStatus::Running;
            LOG_DEBUG("OnZLMStreamState: 流刚启动成功（{}秒前），保持 Running 状态，避免因 ZLM Hook 时序问题导致误判: {} regist={} alive={}",
    ...
    if (old_status != new_status) {
        ...
        if (new_status == StreamStatus::Running) {
            ...
            if (old_status == StreamStatus::Starting) {
                ...
                if (time_since_update >= 30) {
                    ...
                    transition_reason = "流刚启动成功，保持 Running 状态，避免因 ZLM Hook 时序问题导致误判";
                    LOG_DEBUG("OnZLMStreamState: 流刚启动成功（{}秒前），拒绝降级 Running -> Stopped: {} regist={} alive={}",
                             time_since_update, key, regist, alive);
                }
        ...
        if (valid_transition) {
            ...
            LOG_INFO("OnZLMStreamState: 状态转移 {} -> {} ({}): {} regist={} alive={}",
                     static_cast<int>(old_status), static_cast<int>(new_status),
                     transition_reason, key, regist, alive);
            RecordStatusTransition(app, stream, old_status, new_status, "zlm_state_update");
        } else {
            // 非法状态转移：记录告警但允许更新（避免阻塞）
            LOG_WARN("OnZLMStreamState: 检测到潜在的状态转移冲突 {} -> {} ({}): {} regist={} alive={}，"
                     "允许更新但请检查状态机逻辑",
                     static_cast<int>(old_status), static_cast<int>(new_status),
                     key, regist, alive);
            metadata.status = new_status;
...
StreamMetadata StreamManager::GetStreamMetadata(const std::string& app, const std::string& stream) const {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string key = GetStreamKey(app, stream);
        ...
        if (app == "gb28181" && stream.find('_') != std::string::npos) {
            ...
            if (last_underscore != std::string::npos && last_underscore < stream.length() - 1) {
                ...
                if (is_timestamp) {
                    ...
                    it = streams_.find(base_key);
                    if (it != streams_.end()) {
                        LOG_DEBUG("GetStreamMetadata: 使用基础名称找到流: {} -> {}", key, base_key);
                        return it->second;
                    }
                }
            }
...
void StreamManager::ForEachStream(std::function<bool(const StreamMetadata&)> callback) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<StreamMetadata> StreamManager::GetStreamsByProtocol(const std::string& protocol) const {
    std::lock_guard<std::mutex> lock(mutex_);
    std::vector<StreamMetadata> result;
...
std::vector<StreamMetadata> StreamManager::GetStreamsByGatewayType(const std::string& gateway_type) const {
    std::lock_guard<std::mutex> lock(mutex_);
    std::vector<StreamMetadata> result;
...
bool StreamManager::StreamExists(const std::string& app, const std::string& stream) const {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string key = GetStreamKey(app, stream);
...
std::vector<StreamManager::StreamUpdate> StreamManager::PrepareStreamUpdates(
    const std::map<std::string, StreamInfo>& zlm_stream_map,
    int64_t now) {
    ...
    auto snapshot_duration = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    for (const auto& [key, metadata] : snapshot) {
        ...
        if (metadata.gateway_type == "httpflv_gateway") {
            auto zlm_info = zlm_client_->GetStreamInfo(metadata.app, metadata.stream, 
            ...
            if (!zlm_info.app.empty()) {
                ...
                if (time_diff > 0 && zlm_info.total_bytes > metadata.total_bytes) {
                    update.bytes_speed = (zlm_info.total_bytes - metadata.total_bytes) / time_diff;
                ...
                update.zlm_alive = zlm_info.alive || zlm_info.bytes_speed > 0 || zlm_info.total_bytes > 0;
            } else {
        ...
        if (it == zlm_stream_map.end()) {
            for (const auto& schema : {"rtmp", "rtsp", "hls", "ts"}) {
                ...
                it = zlm_stream_map.find(protocol_key);
                if (it != zlm_stream_map.end()) break;
            }
        }
        ...
        if (it != zlm_stream_map.end()) {
            ...
            update.new_status = is_running ? StreamStatus::Running : StreamStatus::Stopped;
    ...
    LOG_INFO("Phase 2.1: PrepareStreamUpdates completed: {} streams, {}μs total (snapshot: {}μs)",
             updates.size(), total_duration, snapshot_duration);
    
    return updates;
}

int StreamManager::ApplyStreamUpdates(
    ...
    for (const auto& update : updates) {
        ...
        // 状态更新
        if (update.new_status.has_value() && *update.new_status != update.old_status) {
            metadata.status = *update.new_status;
            RecordStatusTransition(update.app, update.stream, update.old_status, *update.new_status, update.transition_source);
        }
        ...
        if (update.should_remove) {
             ...
             // 只有状态变化或节流时间到才广播
             bool force = (update.new_status.has_value() && *update.new_status != update.old_status);
    ...
    auto total_apply_duration = std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::steady_clock::now() - apply_start).count();
    
    LOG_INFO("Phase 2.1: ApplyStreamUpdates: {} streams, lock held: {}μs, total: {}μs",
...
int StreamManager::SyncZLMStatus() {
    ...
    // 如果没有指定 output_protocol 的流，或者所有流都是 webrtc，则查询所有协议（向后兼容）
    ...
    for (const auto& schema : schemas_to_query) {
        ...
        zlm_streams.insert(zlm_streams.end(), schema_streams.begin(), schema_streams.end());
    }
    ...
    for (const auto& stream : zlm_streams) {
        std::string key = GetStreamKey(stream.app, stream.stream) + "/" + stream.schema;
        ...
        for (const auto& pair : zlm_stream_map) {
            ...
            if (streams_.count(stream_key) == 0) {
                ...
                if (!zlm_info.origin_url.empty() && 
                    ...
                    // 从origin_url推断协议
                    std::string inferred_protocol = StreamManager::InferProtocolFromSourceUrl(zlm_info.origin_url);
                ...
                LOG_INFO("自动注册新流: {} (协议: {}, 来源: {})", 
                         stream_key, metadata.protocol, 
                         zlm_info.origin_url.empty() ? "unknown" : zlm_info.origin_url.substr(0, 50));
...
void StreamManager::StartStatusSync(int sync_interval_seconds) {
    ...
    sync_thread_ = std::thread(&StreamManager::StatusSyncThread, this, sync_interval_seconds);
...
void StreamManager::StatusSyncThread(int sync_interval_seconds) {
    while (sync_running_.load()) {
        ...
        sleep_cv_.wait_for(lock, std::chrono::seconds(sync_interval_seconds), [this] {
...
nlohmann::json StreamManager::GetStatusStatistics() const {
    ...
    result["total_streams"] = streams_.size(); // Add top-level total_streams field
    result["status_counts"] = {
        {"running", count_running},
        {"starting", count_starting},
        {"stopped", count_stopped},
        {"error", count_error},
        {"stopping", count_stopping},
        {"total", streams_.size()}
    };
    
    result["transition_stats"] = {
        {"gateway_create_requested", status_stats_.transitions.gateway_create_requested},
        {"gateway_create_result", status_stats_.transitions.gateway_create_result},
    ...
    std::vector<StatusStatistics::TransitionDetail> transitions_copy = status_stats_.recent_transitions;
    
    result["recent_transitions"] = nlohmann::json::array();
    size_t start_idx = transitions_copy.size() > 20 ? 
                       transitions_copy.size() - 20 : 0;
    for (size_t i = start_idx; i < transitions_copy.size(); ++i) {
        const auto& trans = transitions_copy[i];
        result["recent_transitions"].push_back({
            {"app", trans.app},
            {"stream", trans.stream},
