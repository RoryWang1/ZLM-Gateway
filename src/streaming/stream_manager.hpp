...
struct StreamMetadata {
    ...
    std::string gateway_type;           // Gateway 类型: "native", "rtsp_gateway", "onvif_gateway", etc.
    std::string processing_type;        // 处理类型: "0"(Direct Proxy), "1"(FFmpeg Copy), "2"(FFmpeg Transcode)
    ...
    // WebSocket 广播控制
    int64_t last_broadcast_time = 0;    // 最后一次WebSocket广播时间（Unix 时间戳，毫秒）
};
...
public:
     ...
     */
    StreamManager(std::shared_ptr<ZLMClient> zlm_client, 
     ...
     * @return Inferred protocol (e.g. "rtsp", "rtmp", "hls") or empty string if unknown
     */
     ...
     * @return 是否成功
     */
    bool UpdateStreamStatus(const std::string& app, 
     ...
     * @return 是否成功
     */
    bool UpdateStreamMetadata(const std::string& app,
                             const std::string& stream,
     ...
     */
    StreamMetadata GetStreamMetadata(const std::string& app, const std::string& stream) const;
     ...
     */
    void ForEachStream(std::function<bool(const StreamMetadata&)> callback) const;
     ...
     * @return 流元数据列表
     */
    std::vector<StreamMetadata> GetStreamsByProtocol(const std::string& protocol) const;
     ...
     * @return 流元数据列表
     */
    std::vector<StreamMetadata> GetStreamsByGatewayType(const std::string& gateway_type) const;
    ...
    void ForEachStream(Func&& callback) const {
     ...
     */
    void OnStreamCreateResult(const std::string& app,
                              const std::string& stream,
                              bool success,
     ...
     */
    void OnZLMStreamState(const std::string& app,
                          const std::string& stream,
                          bool regist,
                          bool alive,
                          int reader_count,
...
private:
     ...
     */
    std::string GetStreamKey(const std::string& app, const std::string& stream) const;
     ...
     */
    void BroadcastStreamUpdate(StreamMetadata& metadata, bool removed = false, bool force = false);

    /**
     * @brief 记录状态转换统计（阶段 C：监控功能）
     * @param from_status 原状态
     * @param to_status 新状态
     * @param source 转换来源（"gateway_create_requested", "gateway_create_result", "zlm_state_update", "sync_polling", "manual_update"）
     */
    void RecordStatusTransition(const std::string& app, const std::string& stream, StreamStatus from_status, StreamStatus to_status, const std::string& source);
     ...
     * @return 更新列表
     */
    std::vector<StreamUpdate> PrepareStreamUpdates(
    ...
    struct StatusStatistics {
        ...
        std::vector<TransitionDetail> recent_transitions;  // 最近的状态转换（保留最近 100 条）
        static constexpr size_t MAX_RECENT_TRANSITIONS = 100;
    } status_stats_;
    mutable std::mutex stats_mutex_;  // 保护统计数据的互斥锁
    std::map<std::string, StreamMetadata> streams_;  // 流元数据存储（key: app/stream）
    std::set<std::string> manually_stopped_streams_;  // 手动停止的流列表（防止自动重新注册）
