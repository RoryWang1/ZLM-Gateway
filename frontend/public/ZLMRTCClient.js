var ZLMRTCClient = (function (exports) {
	...
	class RTCEndpoint extends Event$1 {
		...
		receive() {
			...
			log(this.TAG, 'Transceivers added. Video:', this.options.videoEnable, 'Audio:', this.options.audioEnable);
			...
			this.pc.createOffer().then(desc => {
				...
				this.pc.setLocalDescription(desc).then(() => {
					axios({
						...
						if (this.options.forceTcp) {
							log(this.TAG, 'Forcing TCP, filtering UDP candidates');
							anwser.sdp = anwser.sdp.split('\n').filter(line => {
								return !line.includes('a=candidate') || !line.includes('udp');
							}).join('\n');
							log(this.TAG, 'Filtered answer:', anwser.sdp);
						}
