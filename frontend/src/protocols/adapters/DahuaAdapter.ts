...
export class DahuaAdapter implements IProtocolAdapter {
  readonly protocol: DeviceProtocol = 'dahua';
  ...
  async discover(options: DiscoveryOptions): Promise<Device[]> {
    ...
    const response = await apiClient.post<{ devices: Device[]; count: number }>('/dahua/devices/discover', {
  ...
  async addDevice(device: Partial<Device>): Promise<Device> {
    throw new Error('Dahua device manual add not implemented');
  }

  async deleteDevice(deviceId: string): Promise<void> {
    return apiClient.delete<void>(`/dahua/devices/${deviceId}`);
  }
